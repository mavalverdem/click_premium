...
Private porstcodetrac As ADODB.Recordset
']
...
Private Sub Form_Load()
 ...
 '[Recordsets.                         'Cambiar.
   ...
   Set porstcodetrac = New ADODB.Recordset
...
'     .CursorLocation = adUseClient   'Es el Default.
   ...
   With porstcodetrac
      .ActiveConnection = pocnnMain
      .Source = "SELECT coddetrac, "
      .Source = .Source & Choose(gsIdioma, "detdetrac", "detdetracx") & " AS detdetrac "
      .Source = .Source & "FROM codetrac "
      .Source = .Source & "WHERE codemp='" & gsCodEmp & "' "
'2015-06-26 proceso detrac       .Source = .Source & "AND pdoano='" & gsAnoAct & "'"
'     .CursorLocation = adUseClient   'Es el Default.
 ...
 '[Parámetros.                         'Cambiar.
   With txtDato
      For dnContador = 0 To 1
         .Item(dnContador).DataField = "coddetrac"
         .Item(dnContador).MaxLength = porstcodetrac.Fields(.Item(dnContador).DataField).DefinedSize
      Next
   End With
 ']
  ...
  For nElemento = 0 To UBound(aLabel, 1) - 1
    aLabel(nElemento, 0) = Choose(nElemento + 1, "Diarios")
    aLabel(nElemento, 1) = Choose(nElemento + 1, "Journals")
  Next nElemento
 ...
 '[Datos predeterminados.              'Cambiar.
  'Límites de rangos.
   With porstcodetrac
      .MoveLast
      txtDato(1).Text = !coddetrac
      .MoveFirst
      txtDato(0).Text = !coddetrac
   End With
...
Private Sub cmdImprimir_Click(Index As Integer)
  ...
  With porstMRp
    ...
    '.Source = "SELECT coddetrac, " & Choose(gsIdioma, "detdetrac", "detdetracx") & " AS detdetrac,estdetrac, pctdetrac "
    .Source = "SELECT coddetrac, " & Choose(gsIdioma, "detdetrac", "detdetracx") & " AS detdetrac, pctdetrac "
    .Source = .Source & "FROM codetrac "
    .Source = .Source & "WHERE codemp='" & gsCodEmp & "' "
'2015-06-26 proceso detrac     .Source = .Source & "AND pdoano='" & gsAnoAct & "' "
    .Source = .Source & "AND coddetrac BETWEEN '" & txtDato(0).Text & "' AND '" & txtDato(1).Text & "' "
    .Source = .Source & "ORDER BY coddetrac "
    .Open
  End With
  ...
  If usDEstino = PRN_DEST_GRAF Then
    ...
    With frmMain.rptMain
      ...
      .ReportFileName = gsRutRpt & "rptLDetrac.rpt"
  ...
  Else
    ...
    With MRViewer
      ...
      gpEncabezadoMRp MRViewer, Me.Caption, udFecha, True
...
Private Sub ppAyuBus(tnIndex As Integer)
   Select Case tnIndex
   Case 0, 1                           'Cambiar (añadir índices).
      modAyuBus.Detrac_Cod "", txtDato(tnIndex).Text, 0, 0, Me.Top + fraRangos.Top + txtDato(tnIndex).Top + txtDato(tnIndex).Height, Me.Left + fraRangos.Left + txtDato(tnIndex).Left
   ...
   Case 0, 1
      ...
      With porstcodetrac
         ...
         If .EOF Then
            ...
            ppAyuDet = True
         Else
            lblDatoDeta(tnIndex).Caption = " " & IIf(IsNull(!detdetrac), "", !detdetrac)
         End If
      End With
   End Select
End Function
