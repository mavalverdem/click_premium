...
'     .CursorLocation = adUseClient   'Es el Default.
   ...
   End With
    With porstMRpRs
        ...
        .Source = "SELECT * FROM tmpRptRegPerce"
'        .Open
    End With
   With porstTGAux
      .ActiveConnection = pocnnMain
...
Private Sub cmdImprimir_Click(Index As Integer)
  ...
  With porstMRp
    ...
    .Open
  End With
  ppDatosPercepiones porstMRp.Source
...
'        CadCrystal = "CREATE TABLE Aux2Ret " & porstMRp.Source
...
'            .Formulas(6) = "pSigMon='" & IIf(cboTpoMon.ListIndex = TPOMON_NAC_IND, gsTpoMon_Sgn_ME, gsTpoMon_Sgn_MN) & "'"
            ...
            .Action = 1
'            pocnnMain.Execute ("Drop Table HONORARIOS")
        End With
'        If Index = 0 Then frmMain.rptMain.Destination = crptToWindow
    ...
    End If
   pocnnMain.Execute "DROP TABLE IF EXISTS tmpRptRegPerce"
   porstMRpRs.Close
...
Public Sub ppDatosPercepiones(s_Source As String)
   Dim dorstPercepcion As ADODB.Recordset
   Dim dorstCOCpbDetDR As ADODB.Recordset
   Dim dorstCOCpbDetI As ADODB.Recordset
   Dim dsNroIte As Integer, dsNroGrp As Integer
   
   Set dorstPercepcion = New ADODB.Recordset
   Set dorstCOCpbDetDR = New ADODB.Recordset
   Set dorstCOCpbDetI = New ADODB.Recordset
   pocnnMain.Execute "DROP TABLE IF EXISTS tmpRptRegPerce"
   pocnnMain.Execute "CREATE TABLE IF NOT EXISTS tmpRptRegPerce " & s_Source
   pocnnMain.Execute "DELETE FROM tmpRptRegPerce"
   porstMRpRs.Open
   ...
   With dorstCOCpbDetDR
      ...
      .Source = "SELECT DISTINCTROW CONCAT(a.CodAux, a.CodCta, a.CodTDc, a.SerDoc, a.NroDoc) as cLlaveD " _
              & "FROM (CoCpbDet a " _
              & "   LEFT JOIN COCpbDetRP b " _
              & "   ON a.CodAux=b.CodAux AND a.CodCta=b.CodCta AND a.CodTDc=b.CodTDc " _
              & "   AND a.SerDoc=b.SerDoc AND a.NroDoc=b.NroDoc AND a.NroIte=b.NroIte) " _
              & "WHERE (a.TpoGnr<>'" & TPOGNR_DST & "') AND b.CodTDc_RP='" & gsCodTDc_Pcp & "' " _
   ...
   With dorstCOCpbDetI
      ...
      .Source = "SELECT DISTINCTROW a.CodAux, c.RazAux, c.RUCAux, a.FehOpe, a.NroIte, " _
              ...
              & "  IF(a.TpoPvs='C', 'Pago', IF(a.TpoPvs='P', 'Venta', 'Ajuste')) as cTpoTra, " _
      ...
      .Open
   End With
   With dorstPercepcion
      .ActiveConnection = pocnnMain
      .Source = "SELECT DISTINCTROW a.FehOpe, " _
              & "  CONCAT(a.SerDoc, '-', a.NroDoc) AS cDocumento, 'C/Perc' as cDenCpb, 'Percepción' as cTpoTra, " _
              & "  a.TpoCtb, a.ImpMN, a.ImpME, a.ImpTCb, a.CodTDc, " _
              & "  CONCAT(b.CodTDc_RP, b.SerDoc_RP, b.NroDoc_RP) AS cDocR " _
              & "FROM (CoCpbDet a " _
              & "   LEFT JOIN COCpbDetRP b ON a.CodTDc=b.CodTDc_RP AND a.SerDoc=b.SerDoc_RP AND a.NroDoc=b.NroDoc_RP) " _
              & "WHERE a.CodTDc='" & gsCodTDc_Pcp & "' " _
      ...
      Do While Not .EOF
         ...
         If Not dorstCOCpbDetI.EOF Then
            ...
            Do While Not dorstCOCpbDetI.EOF
               ...
               Else
                  If dorstPercepcion.RecordCount > 0 Then dorstPercepcion.MoveFirst
                  dorstPercepcion.Find "cDocR='" & dorstCOCpbDetI!cDocR & "'"
                  If Not dorstPercepcion.EOF Then
                     PPAddrstMRP dorstCOCpbDetI!CodAux, dorstCOCpbDetI!RazAux, dorstCOCpbDetI!RucAux, dsNroIte, dorstCOCpbDetI!FehOpe, dorstCOCpbDetI!FehOpe, dorstCOCpbDetI!cDocumento, _
                     ...
                     dsNroIte = dsNroIte + 1
                     PPAddrstMRP dorstCOCpbDetI!CodAux, dorstCOCpbDetI!RazAux, dorstCOCpbDetI!RucAux, dsNroIte, dorstPercepcion!FehOpe, dorstPercepcion!FehOpe, dorstPercepcion!cDocumento, _
                              dorstPercepcion!cDenCpb, dorstPercepcion!cTpoTra, dorstPercepcion!TpoCtb, dorstPercepcion!ImpMN, _
                              dorstPercepcion!ImpME, dorstPercepcion!CodTDc, dsNroGrp, dorstPercepcion!ImpTCb
                     dsNroIte = dsNroIte + 1
                     PPAddrstMRP dorstCOCpbDetI!CodAux, dorstCOCpbDetI!RazAux, dorstCOCpbDetI!RucAux, dsNroIte, dorstPercepcion!FehOpe, dorstPercepcion!FehOpe, dorstPercepcion!cDocumento, _
                              "Percepción", "Percepción", IIf(dorstPercepcion!TpoCtb = TPOCTB_DEB, TPOCTB_HAB, TPOCTB_DEB), dorstPercepcion!ImpMN, _
                              dorstPercepcion!ImpME, dorstPercepcion!CodTDc, dsNroGrp, dorstPercepcion!ImpTCb
                  Else
                     PPAddrstMRP dorstCOCpbDetI!CodAux, dorstCOCpbDetI!RazAux, dorstCOCpbDetI!RucAux, dsNroIte, dorstCOCpbDetI!FehOpe, dorstCOCpbDetI!FehOpe, dorstCOCpbDetI!cDocumento, _
                              dorstCOCpbDetI!cDenCpb, dorstCOCpbDetI!cTpoTra, dorstCOCpbDetI!TpoCtb, CDec(dorstCOCpbDetI!ImpMN), _
