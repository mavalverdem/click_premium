...
Private Sub Form_Load()
   ...
   With frmMFilGrd                     'Cambiar Formulario de Grid.
    '[Llaves                           'Cambiar
      txtLlave(0).MaxLength = .uorstMain_0!codfil.DefinedSize
    ']
   
    '[Datos.                           'Cambiar.
      txtDato(gsIdioma - 1).MaxLength = .uorstMain_0!detfil.DefinedSize
      txtDato(2 - gsIdioma).MaxLength = .uorstMain_0!detfilx.DefinedSize
      txtDato(2).MaxLength = .uorstMain_0!sepcar.DefinedSize
    ']
   End With
  ...
  For nElemento = 0 To UBound(aLabel, 1) - 1
    aLabel(nElemento, 0) = Choose(nElemento + 1, "Plantilla Archivo:", "Descripción:", "Traducción:", "Carácter :")
    aLabel(nElemento, 1) = Choose(nElemento + 1, "Structure File:", "Description:", "Translation:", "Character :")
  Next nElemento
...
Private Sub Form_Unload(Cancel As Integer)
  If Not (frmMFilGrd.uorstMain_0.BOF And frmMFilGrd.uorstMain_0.EOF) Then
   frmMFilGrd.uorstMain_0.CancelUpdate 'Cambiar Formulario de Grid.
  End If
End Sub

Private Sub cmdRetroceder_Click()
   gpTUe_Retroceder frmMFilGrd.uorstMain_0, Me 'Cambiar Formulario de Grid.
End Sub

Private Sub cmdAvanzar_Click()
   gpTUe_Avanzar frmMFilGrd.uorstMain_0, Me 'Cambiar Formulario de Grid.
End Sub
...
Public Sub cmdGrabar_Click()
  ...
  '[Validacion de Datos
  If Trim(txtDato(2).Text) = "" Then MsgBox TEXT_8005, vbExclamation: txtDato(2).SetFocus: Exit Sub
   With frmMFilGrd                     'Cambiar Formulario de Grid.
...
'      .uorstCCCfg.Update
      ...
      If pbNuevo Then
       ...
       '[Búsqueda de llave actual.     'Cambiar.
         .uorstMain_0.Find "codfil='" & txtLlave(0).Text & "'"
       ']
         cmdGrabar.Enabled = False
         upHabilitacion False
...
'   End Select
  ...
  'Valida la llave.                    'Cambiar.
   If Len(Trim(txtLlave(Index).Text)) <> 0 Then
      With frmMFilGrd.uorstMain_0        'Cambiar Formulario de Grid.
         If Not (.BOF And .EOF) Then
            dvRegistro = .Bookmark
            .MoveFirst
            .Find "codfil='" & txtLlave(0).Text & "'"
            If Not .EOF Then
      ...
      If tnFase = 0 Then
        'Llaves.
         If pbNuevo Then
            ...
            .uorstMain_0!codfil = txtLlave(0).Text
         End If

        'Datos.
         .uorstMain_0!detfil = txtDato(gsIdioma - 1).Text
         .uorstMain_0!detfilx = txtDato(2 - gsIdioma).Text
         .uorstMain_0!sepcar = txtDato(2).Text
      Else
        'Llaves.
         txtLlave(0).Text = .uorstMain_0!codfil
      
        'Datos.
         txtDato(gsIdioma - 1).Text = IIf(IsNull(.uorstMain_0!detfil), "", .uorstMain_0!detfil)
         txtDato(2 - gsIdioma).Text = IIf(IsNull(.uorstMain_0!detfilx), "", .uorstMain_0!detfilx)
         txtDato(2).Text = IIf(IsNull(.uorstMain_0!sepcar), "", .uorstMain_0!sepcar)
