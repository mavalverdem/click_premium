...
Private porstCoFjo As ADODB.Recordset
']
...
'     .CursorLocation = adUseClient   'Es el Default.
   ...
   With porstCoFjo
      .ActiveConnection = pocnnMain
      .Source = "SELECT CodFjo, " & Choose(gsIdioma, "DetFjo", "DetFjox") & " AS DetFjo "
...
'     .CursorLocation = adUseClient   'Es el Default.
 ...
 '[Parámetros.                         'Cambiar.
   With txtDato
      For dnContador = 0 To 1
         .Item(dnContador).DataField = "CodFjo"
         .Item(dnContador).MaxLength = porstCoFjo.Fields(.Item(dnContador).DataField).DefinedSize
      Next
   End With
 ']
  ...
  For nElemento = 0 To UBound(aLabel, 1) - 1
    aLabel(nElemento, 0) = Choose(nElemento + 1, "Flujo")
    aLabel(nElemento, 1) = Choose(nElemento + 1, "Flow")
  Next nElemento
  fraRangos.Caption = Choose(gsIdioma, "Rango de Flujo de Caja", "Range Cash Flow")
 ...
 '[Datos predeterminados.              'Cambiar.
  'Límites de rangos.
   With porstCoFjo
      .MoveLast
      txtDato(1).Text = !CodFjo
      .MoveFirst
      txtDato(0).Text = !CodFjo
   End With
...
Private Sub cmdImprimir_Click(Index As Integer)
  ...
  With porstMRp
    ...
    .Source = "SELECT CodFjo, " & Choose(gsIdioma, "DetFjo", "DetFjox") & " AS DetFjo, TpoFjo, codefe "
    .Source = .Source & "FROM COFjo "
    .Source = .Source & "WHERE codemp='" & gsCodEmp & "' "
    .Source = .Source & "AND pdoano='" & gsAnoAct & "' "
    .Source = .Source & "AND CodFjo BETWEEN '" & txtDato(0).Text & "' AND '" & txtDato(1).Text & "' "
  ...
  If usDEstino = PRN_DEST_GRAF Then
    ...
    With frmMain.rptMain
      ...
      .ReportFileName = gsRutRpt & "rptLFjo.rpt"
  ...
  Else
    ...
    With MRViewer
      ...
      .LoadReport gsRutRpt & "rptLFjo.mrp"
...
Private Sub ppAyuBus(tnIndex As Integer)
   Select Case tnIndex
   Case 0, 1                           'Cambiar (añadir índices).
      modAyuBus.Fjo_Cod "", txtDato(tnIndex).Text, 0, 0, Me.Top + fraRangos.Top + txtDato(tnIndex).Top + txtDato(tnIndex).Height, Me.Left + fraRangos.Left + txtDato(tnIndex).Left
