...
'      .ConnectionString = CONNSTRG  & gsRutBDS & gsNomBDS
   ...
   With porstCOCprDoc
      ...
      .Source = .Source & "AND (concat(a.pdoano,a.mespvs)>='" & gsAnoAct & "01' " & " AND concat(a.pdoano,a.mespvs)<='" & Trim(Str(Val(gsAnoAct) + 1)) & "02') " & " AND year(a.feedoc)='" & gsAnoAct & "' "
...
''   pgbEtapa1.Max = 4
   ...
   With porstCOVtaDoc
      ...
      If ps_Plataforma = pSrvMySql Then
        .Source = .Source & "HAVING Total>" & (CDec(gnImpUIT) * 2) & " "
      ElseIf ps_Plataforma = pSrvSql Then
        .Source = .Source & "HAVING SUM((CASE d.SgnTDc WHEN " & SGNTDC_NEG & " THEN (a.ImpOGr_MN+a.ImpExp_MN+a.ImpExo_mn) * -1 ELSE (a.ImpOGr_MN+a.ImpExp_MN+a.ImpExo_mn END)))>" & (CDec(gnImpUIT) * 2) & " "
      End If
'     .CursorLocation = adUseClient   'Es el Default.
      .CursorType = adOpenDynamic
      .LockType = adLockBatchOptimistic
