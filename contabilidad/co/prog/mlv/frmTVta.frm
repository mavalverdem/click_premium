...
Begin VB.Form frmTVta 
   ...
   Begin TabDlg.SSTab sstMain 
      ...
      Tab(2).Control(0)=   "fraExterior"
      Tab(2).Control(1)=   "fraPercepcion"
      Tab(2).Control(2)=   "fraReferencia"
      Tab(2).Control(3)=   "fraDetracion"
      Tab(2).ControlCount=   4
      TabCaption(3)   =   "Sunat"
      TabPicture(3)   =   "frmTVta.frx":181E
      Tab(3).ControlEnabled=   0   'False
      Tab(3).Control(0)=   "lblDatoDeta(43)"
      Tab(3).Control(1)=   "lblTexto(42)"
      Tab(3).Control(2)=   "txtDato(43)"
      Tab(3).Control(3)=   "cmdDatoAyud(43)"
...
'    sSQL = sSQL & "'" & porstRegistro!forimp & "', "
  ...
  With porstRegistro
    ...
    .Open
  End With
'ini 2016-05-27/28 reporte prueba mashall
  Dim xrpt As Integer
  'xrpt = 1 '1=mashall 0=crystal
  xrpt = 0 '1=mashall 0=crystal
'fin 2016-05-27/28 reporte prueba mashall
...
'ini 2016-05-27/28 reporte prueba mashall
    ...
    With MRViewer
      ...
      .DataRecordSet = porstRegistro
      '.LoadReport gsRutRpt & sReporte & ".mrp"
      .LoadReport gsRutRpt & "rptdocvcenta" & nFormato & ".mrp"
...
''fin 2015-07-08 adic tabla detrac
    txtDato(44).MaxLength = .uorstMain!tsadetrac.DefinedSize
...
'ini 2016-03-30 elimina cboDetraccion
    ...
    If txtDato(44).Text = "" Then MsgBox TEXT_6002, vbCritical: txtDato(44).SetFocus: Exit Sub
'fin 2016-03-30 elimina cboDetraccion
  End If
'fin 2015-07-07 detrac vtas
...
'ini 2016-03-30.31 elimina cboDetraccion
    Case 44
        txtDato(Index).SetFocus
'fin 2016-03-30.31 elimina cboDetraccion
   End Select
...
'      End If
   ...
   Case 33, 43                            'Cambiar (añadir índices).
      ...
      If Cancel Then Exit Sub
'ini 2016-03-30.31 elimina cboDetraccion
   Case 44                           'Cambiar (añadir índices).
      Cancel = ppAyuDet(AYUDAT, Index)
      If Cancel Then Exit Sub
'fin 2016-03-30.31 elimina cboDetraccion
   End Select
...
'         End With
''fin 2015-07-08 adic tabla detrac
      !tsadetrac = IIf(txtDato(44).Text = "", Null, txtDato(44).Text)
'fin 2016-03-30.31 elimina cboDetraccion
...
'ini 12-09-2016 corrige glosa vta
...
'fin 12-09-2016 corrige glosa vta
...
'      End If
''ini 2015-07-08 detrac vtas
    txtDato(44).Text = IIf(IsNull(!tsadetrac), "", !tsadetrac)
    ppAyuDet AYUDAT, 44
'fin 2016-03-30.31 elimina cboDetraccion
...
''fin 2015-07-08 detrac vtas
    txtDato(44).Enabled = tbHabilitar 'fin 2016-02-02.03 correccion ple
   cmdDatoAyud(44).Enabled = tbHabilitar
   lblDatoDeta(44).Enabled = tbHabilitar
'fin 2016-03-30.31 elimina cboDetraccion
...
'2016-07-27 valida cta y ccosto detalle Private Sub ppAbreCtaCCo()
Private Sub ppAbreCtaCCo(Optional ByVal pOrder As Integer)
   ...
   With frmTVtaGrd.uorstCOVtaDocCta
    ...
    If .State = adStateOpen Then .Close
'ini 2016-07-27 valida cta y ccosto detalle
'    .Source = frmTVtaGrd.usConnStrgSele_COVtaDocCta & frmTVtaGrd.usConnStrgWher_COVtaDocCta & frmTVtaGrd.usConnStrgOrde_COVtaDocCta
    .Source = frmTVtaGrd.usConnStrgSele_COVtaDocCta & frmTVtaGrd.usConnStrgWher_COVtaDocCta
    If pOrder = 0 Then
    .Source = .Source & frmTVtaGrd.usConnStrgOrde_COVtaDocCta
    Else
        .Source = .Source & " ORDER BY TpoCnc,CodCta, Orden"
    End If
'fin 2016-07-27 valida cta y ccosto detalle
...
'ini 2016-07-13 error duplica nro cmpbte 2da vez graba vta
            If Not .EOF Then .Delete
'quedo pendiente por que teo debe decidir
'            If Not .EOF Then
'            .Delete
'            'txtDato(1).Text = ""
'            Dim xsql As String
'            xsql = "UPDATE covtadoc SET nrocpb=null "
'            xsql = xsql & "WHERE codemp='" & gsCodEmp & "' AND pdoano='" & gsAnoAct & "' "
'            xsql = xsql & "AND MesPvs='" & gsMesAct & "' AND CodAux='" & txtDato(33).Text & "' "
'            xsql = xsql & "AND CodTDc='" & txtLlave(0).Text & "' AND SerDoc='" & txtLlave(1).Text & "'"
'            xsql = xsql & "AND NroDoc='" & txtLlave(2).Text & "'"
'            'xsql = xsql & "AND NroDoc='" & uorstMain_Grd!nrodoc & "' AND TpoPvs<>'" & TPOPVS_CAN & "'"
'            frmTVtaGrd.uocnnMain.Execute xsql
'            End If
'fin 2016-07-13 error duplica nro cmpbte 2da vez graba vta
         End If
      End With
      End If
   End If
...
'solo es una prueba
'''ini 2016-08-12 error valida compra
''          If MINIMOINDICEIMPORTEMN + dnContador = 5 Then
''          frmTVtaGrd.ubGrabaMas = 1
''          End If
'''fin 2016-08-12 error valida compra
