...
Public uorstCOEfe As ADODB.Recordset
']

Private Sub Form_Load()
 '[Recordsets                          'Cambiar.
'   psConnStrgSele = "SELECT a.CodDro, a.DetDro," _
                  & "  a.UsrCre, a.FyHCre, a.UsrMdf, a.FyHMdf " _
                  & "FROM CoDro as a "
   psConnStrgSele = "SELECT CodEfe, "
   psConnStrgSele = psConnStrgSele & Choose(gsIdioma, "DetEfe, ", "DetEfex, ")
   psConnStrgSele = psConnStrgSele & Choose(gsIdioma, "DetEfex, ", "DetEfe, ")
   psConnStrgSele = psConnStrgSele & "TpoEfe, codemp, pdoano, "
   psConnStrgSele = psConnStrgSele & "UsrCre, FyHCre, UsrMdf, FyHMdf "
   psConnStrgSele = psConnStrgSele & "FROM CoEfe "
...
'     .CursorLocation = adUseClient   'Es el Default.
      ...
      .Open
      .Properties("Unique Table").Value = "CoEfe"
'      .Properties("Unique Table").Value = "a"
   End With
   With uorstCOEfe
      .ActiveConnection = uocnnMain
      .Source = "SELECT a.CodEfe, a.DetEfe, a.DetEfex "
      .Source = .Source & "FROM COEfe a "
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmMEfe             'Cambiar Formulario de Datos.
End Sub
...
Public Sub cmdRevisar_click()
   ...
   With frmMEfe                        'Cambiar Formulario de Datos.
      .zbNuevo = False
...
Dim dcCodEli As String
   ...
   '[No Pertenece al Formulario - Agregado por Angel
   dcCodEli = frmMEfeGrd.uorstMain!CodEfe
   If Len(Trim(dcCodEli)) = 2 Then
      With uorstCOEfe
        ...
        .Source = .Source & "WHERE a.codemp='" & gsCodEmp & "' "
        .Source = .Source & "AND a.pdoano='" & gsAnoAct & "' "
        .Source = .Source & "AND Left(a.CodEfe,2)='" & dcCodEli & "'"
         .Open
      End With
      If uorstCOEfe.RecordCount > 1 Then
...
Public Sub cmdImprimir_Click(Index As Integer)
 ...
 '[Datos del formulario de impresión.  'Cambiar.
   frmLEfe.Caption = Choose(gsIdioma, "Listado de ", "Listing of ") & Me.Caption
   frmLEfe.Show vbModal
 ']
End Sub
...
'            .Item(dnNum).Width = 600
             .Item(dnNum).Width = 100 * (uorstMain.Fields("CodEfe").DefinedSize + 4)
         Case 1
            .Item(dnNum).Caption = Choose(gsIdioma, "Descripción", "Description")
'            .Item(dnNum).Width = 1500
             .Item(dnNum).Width = 100 * (uorstMain.Fields("DetEfe").DefinedSize)
         Case Else
