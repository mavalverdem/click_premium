...
Private Sub Form_Load()
 ...
 '[Recordsets                          'Cambiar.
  psConnStrgSele = "SELECT cntbkeyocur, desdro, coddro1, coddro2, trfdro"
  psConnStrgSele = psConnStrgSele & " FROM tmpdro "
  ...
  s_Sentencia = "SELECT COUNT(*) AS nExiste FROM dba_tables WHERE table_name='" & UCase("tmpdro") & "'"
  ...
  If nRegistro = 0 Then
    s_Sentencia = "CREATE TABLE tmpdro("
    s_Sentencia = s_Sentencia & " cntbkeyocur char(17) NOT NULL,"
    s_Sentencia = s_Sentencia & " coddro1 char(4),"
    s_Sentencia = s_Sentencia & " coddro2 char(4),"
    s_Sentencia = s_Sentencia & " desdro char(40),"
    s_Sentencia = s_Sentencia & " trfdro char(1) DEFAULT '1',"
    s_Sentencia = s_Sentencia & " PRIMARY KEY (cntbkeyocur))"
    uocnnMain.Execute s_Sentencia, nRegistro
  End If
  
  ' Inserto registros no existentes
  s_Sentencia = "INSERT INTO tmpdro(cntbkeyocur, desdro)"
  s_Sentencia = s_Sentencia & " SELECT DISTINCT cntbkeyocur, LPAD(cntbdatocur, 40) AS desdro"
  s_Sentencia = s_Sentencia & " FROM cndmtabl tmp"
  s_Sentencia = s_Sentencia & " WHERE cntbcodtabl=10"
  s_Sentencia = s_Sentencia & " AND NOT EXISTS(SELECT * FROM tmpdro dro WHERE tmp.cntbkeyocur=dro.cntbkeyocur)"
  ...
  With uorstMain
    ...
    .Open
    .Properties("Unique Table").Value = "tmpdro"
  End With
  With uorstCODro
    .ActiveConnection = uocnnMain
    .Source = "SELECT a.cntbkeyocur, a.desdro "
    .Source = .Source & "FROM tmpdro a"
    '     .CursorLocation = adUseClient   'Es el Default.
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmMeDro             'Cambiar Formulario de Datos.
End Sub
...
Public Sub cmdRevisar_click()
   ...
   With frmMeDro                        'Cambiar Formulario de Datos.
      .zbNuevo = False
...
Dim dcCodEli As String
   ...
   '[No Pertenece al Formulario - Agregado por Angel
   dcCodEli = frmMeDroGrd.uorstMain!cntbkeyocur
   If Len(Trim(dcCodEli)) = 2 Then
      With uorstCODro
         .Close
         .Source = "SELECT a.cntbkeyocur, a.desdro " _
                 & "FROM tmpdro a Where Left(a.coddro,2)='" & dcCodEli & "'"
         .Open
      End With
      If uorstCODro.RecordCount > 1 Then
         MsgBox "Existen Diarios Relacionados, No podra Eliminar", vbExclamation
         With uorstCODro
            .Close
            .Source = "SELECT a.cntbkeyocur, a.desdro" _
                    & "FROM tmpdro a"
      ...
      For dnNum = 0 To .Count - 1
         ...
         Case 0
            .Item(dnNum).Caption = "Código"
             .Item(dnNum).Width = 100 * (uorstMain.Fields("cntbkeyocur").DefinedSize - 6)
         Case 1
            .Item(dnNum).Caption = "Descripción"
             .Item(dnNum).Width = 100 * (uorstMain.Fields("desdro").DefinedSize)
         Case 2
            .Item(dnNum).Caption = "Equival1"
             .Item(dnNum).Width = 80 * (uorstMain.Fields("coddro1").DefinedSize + 6)
         Case 3
            .Item(dnNum).Caption = "Equival2"
             .Item(dnNum).Width = 80 * (uorstMain.Fields("coddro1").DefinedSize + 6)
         Case 4
            .Item(dnNum).Caption = "Transf"
