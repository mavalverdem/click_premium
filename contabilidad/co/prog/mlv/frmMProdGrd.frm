...
Private Sub Form_Load()
  Me.Caption = "Productos de Compras"
  
  '[Recordsets                          'Cambiar.
  psConnStrgSele = "SELECT codprod, " & Choose(gsIdioma, "detprod, ", "detprodx, ")
  psConnStrgSele = psConnStrgSele & "unimed, (CASE tpomon WHEN '" & TPOMON_NAC & "' THEN 'S/.' ELSE 'US$' END) AS sTpoMon, impcpr, "
  psConnStrgSele = psConnStrgSele & "(CASE estprod WHEN '" & ESTCCO_ACT & "' THEN '" & ESTCCO_ACT_TXT & "' ELSE '" & ESTCCO_INA_TXT & "' END) AS sestprod, "
  psConnStrgSele = psConnStrgSele & "codemp, pdoano, usrcre, fyhcre, usrmdf, fyhmdf, "
  psConnStrgSele = psConnStrgSele & Choose(gsIdioma, "detprodx", "detprod") & ", estprod, codcta, tpomon "
  psConnStrgSele = psConnStrgSele & "FROM cocprprod "
  ...
  With uorstMain
    ...
    .Open
    .Properties("Unique Table").Value = "cocprprod"
  End With
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmMProd             'Cambiar Formulario de Datos.
End Sub
  ...
  With dgrMain.Columns
    For dnNum = 0 To .Count - 1
      Select Case dnNum
       ...
       Case 2
        .Item(dnNum).Caption = Choose(gsIdioma, "Unidad", "Unit")
        .Item(dnNum).Width = 500
       Case 3
        .Item(dnNum).Caption = Choose(gsIdioma, "Moneda", "Currency")
        .Item(dnNum).Width = 450
       Case 4
