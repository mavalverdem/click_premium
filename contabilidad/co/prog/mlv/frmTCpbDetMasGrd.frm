...
Begin VB.Form frmTCpbDetMasGrd 
   Caption         =   "[Entidad]"
   ClientHeight    =   4665
   ClientLeft      =   165
   ClientTop       =   345
   ...
   Begin VB.PictureBox picOpciones 
      ...
      Begin VB.CommandButton cmdRefrescar 
         ...
         Height          =   560
         Left            =   2160
         Picture         =   "frmTCpbDetMasGrd.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   4
         Top             =   0
         Width           =   720
      ...
      Begin VB.CommandButton cmdSalir 
         ...
         Height          =   560
         Left            =   6390
         Picture         =   "frmTCpbDetMasGrd.frx":014A
         Style           =   1  'Graphical
         TabIndex        =   7
         Top             =   0
         Width           =   720
      ...
      Begin VB.CommandButton cmdNuevo 
         ...
         Height          =   560
         Left            =   0
         Picture         =   "frmTCpbDetMasGrd.frx":0294
         Style           =   1  'Graphical
         TabIndex        =   1
         Top             =   0
         Width           =   720
      ...
      Begin VB.CommandButton cmdEliminar 
         ...
         Height          =   560
         Left            =   1440
         Picture         =   "frmTCpbDetMasGrd.frx":0396
         Style           =   1  'Graphical
         TabIndex        =   3
         Top             =   0
         Width           =   720
      ...
      Begin VB.CommandButton cmdImprimir 
         ...
         Height          =   560
         Left            =   2880
         Picture         =   "frmTCpbDetMasGrd.frx":0498
         Style           =   1  'Graphical
         TabIndex        =   5
         Top             =   0
         Width           =   720
      ...
      Begin VB.CommandButton cmdRevisar 
         ...
         Height          =   560
         Left            =   720
         Picture         =   "frmTCpbDetMasGrd.frx":059A
         Style           =   1  'Graphical
         TabIndex        =   2
         Top             =   0
         Width           =   720
   ...
   End
End
Attribute VB_Name = "frmTCpbDetMasGrd"
...
Private Sub Form_Load()
 '[Recordsets                          'Cambiar.
   psConnStrgSele_Grd = "SELECT " & ps_Prefijo & "tmpCoCpbDetFjo.CodFjo, " & Choose(gsIdioma, "CoFjo.DetFjo", "CoFjo.DetFjox") & ", ImpMN, ImpME, MesPvs, CodDro, NroCpb, "
   psConnStrgSele_Grd = psConnStrgSele_Grd & "NroIte, NroOrd, CodCta, TpoCtb, "
   psConnStrgSele_Grd = psConnStrgSele_Grd & IIf(ps_Plataforma = pSrvMySql, "CONCAT(MesPvs, CodDro, NroCpb, NroIte, NroOrd, tmpCoCpbDetFjo.CodFjo)", "(MesPvs+CodDro+NroCpb+RTrim(NroIte)+RTrim(NroOrd)+#tmpCoCpbDetFjo.CodFjo)") & " AS cLlave, "
   psConnStrgSele_Grd = psConnStrgSele_Grd & ps_Prefijo & "tmpCoCpbDetFjo.UsrCre, " & ps_Prefijo & "tmpCoCpbDetFjo.FyHCre, " & ps_Prefijo & "tmpCoCpbDetFjo.UsrMdf, " & ps_Prefijo & "tmpCoCpbDetFjo.FyHMdf, "
   psConnStrgSele_Grd = psConnStrgSele_Grd & ps_Prefijo & "tmpCoCpbDetFjo.codemp, " & ps_Prefijo & "tmpCoCpbDetFjo.pdoano "
   psConnStrgSele_Grd = psConnStrgSele_Grd & "FROM (" & ps_Prefijo & "tmpCoCpbDetFjo "
   psConnStrgSele_Grd = psConnStrgSele_Grd & "LEFT JOIN CoFjo ON " & ps_Prefijo & "tmpCoCpbDetFjo.codemp=CoFjo.codemp AND " & ps_Prefijo & "tmpCoCpbDetFjo.pdoano=CoFjo.pdoano AND " & ps_Prefijo & "tmpCoCpbDetFjo.CodFjo=CoFjo.CodFjo) "
   ...
   psConnStrgSele = psConnStrgSele & "FROM " & ps_Prefijo & "tmpCoCpbDetFjo "
...
'     .CursorLocation = adUseClient   'Es el Default.
      ...
      .Open
      .Properties("Unique Table").Value = ps_Prefijo & "tmpCoCpbDetFjo"
   End With
   With uorstMain_Grd
      .ActiveConnection = frmTCpbGrd.uocnnMain
      .Source = psConnStrgSele_Grd & psConnStrgOrde
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmTCpbDetMas             'Cambiar Formulario de Datos.
End Sub
...
Public Sub cmdRevisar_click()
 ...
 ']
   With frmTCpbDetMas                        'Cambiar Formulario de Datos.
      .zbNuevo = False
...
Public Sub cmdEliminar_Click()
   ...
   If MsgBox(TEXT_1021 & " " & Trim(dgrMain.Columns(0)) & " (" & Trim(dgrMain.Columns(1)) & ")?", vbYesNo + vbQuestion + vbDefaultButton2, Me.Caption) = vbYes Then
     ...
     uorstMain.Properties("Unique Table").Value = ps_Prefijo & "tmpCoCpbDetFjo"
     uorstMain.Delete
...
Private Sub cmdSalir_Click()
  ...
  nTotalMN = CDec(frmTCpbDet.txtImporte(0).Text) + CDec(frmTCpbDet.txtImporte(1).Text)
  nTotalME = CDec(frmTCpbDet.txtImporte(2).Text) + CDec(frmTCpbDet.txtImporte(3).Text)
  ' Inicializo los datos de flujo
  frmTCpbDet.txtDato(9).Text = ""
  frmTCpbDet.lblDatoDeta(4).Caption = ""
  frmTCpbDet.txtDato(9).Enabled = True
  frmTCpbDet.cmdDatoAyud(5).Enabled = True
  frmTCpbDet.cmdMasFjo.Tag = 0
  If Not (uorstMain_Grd.EOF And uorstMain_Grd.BOF) Then
    ...
    With uorstTmp
      ...
      .Source = .Source & "AND NroIte=" & frmTCpbDet.pnNroIte
'      .CursorLocation = adUseClient   'Es el Default.
    ...
    frmTCpbDet.txtDato(9).Text = uorstMain!CodFjo
    frmTCpbDet.txtDato(9).Enabled = False
    frmTCpbDet.cmdDatoAyud(5).Enabled = False
    frmTCpbDet.cmdMasFjo.Tag = 2
    frmTCpbDet.cmdMasFjo.Enabled = True
  End If
