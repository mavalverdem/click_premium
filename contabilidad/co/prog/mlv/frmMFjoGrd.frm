...
Public uorstCOFjo As ADODB.Recordset, _
       uorstCOEfe As ADODB.Recordset
']

Private Sub Form_Load()
 '[Recordsets                          'Cambiar.
   psConnStrgSele = "SELECT CodFjo, "
   psConnStrgSele = psConnStrgSele & Choose(gsIdioma, "DetFjo, ", "DetFjox, ")
   psConnStrgSele = psConnStrgSele & "TpoFjo, CodEfe, "
   psConnStrgSele = psConnStrgSele & Choose(gsIdioma, "DetFjox, ", "DetFjo, ")
   psConnStrgSele = psConnStrgSele & "codemp, pdoano, "
   psConnStrgSele = psConnStrgSele & "UsrCre, FyHCre, UsrMdf, FyHMdf "
   psConnStrgSele = psConnStrgSele & "FROM CoFjo "
   ...
   Set uorstCOEfe = New ADODB.Recordset
   With uocnnMain
...
'     .CursorLocation = adUseClient   'Es el Default.
      ...
      .Open
      .Properties("Unique Table").Value = "COFJO"
'      .Properties("Unique Table").Value = "a"
   End With
   With uorstCOFjo
      .ActiveConnection = uocnnMain
      .Source = "SELECT a.CodFjo, a.DetFjo, a.DetFjox "
...
'     .CursorLocation = adUseClient   'Es el Default.
   ...
   With uorstCOEfe
      .ActiveConnection = uocnnMain
      .Source = "SELECT CodEfe, DetEfe, DetEfex "
      .Source = .Source & "FROM COEfe "
      .Source = .Source & "WHERE codemp='" & gsCodEmp & "' "
      .Source = .Source & "AND pdoano='" & gsAnoAct & "' "
      .Source = .Source & "AND " & IIf(ps_Plataforma = pSrvMySql, "Length", "Len") & "(CodEfe) > 2"
''     .CursorLocation = adUseClient   'Es el Default.
      .CursorType = adOpenDynamic
      .LockType = adLockOptimistic
      .Open
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmMFjo             'Cambiar Formulario de Datos.
End Sub
...
Public Sub cmdRevisar_click()
   ...
   With frmMFjo                        'Cambiar Formulario de Datos.
      .zbNuevo = False
...
Dim dcCodEli As String
   ...
   '[No Pertenece al Formulario - Agregado por Angel
   dcCodEli = frmMFjoGrd.uorstMain!CodFjo
   If Len(Trim(dcCodEli)) = 2 Then
      With uorstCOFjo
        ...
        .Source = .Source & "WHERE a.codemp='" & gsCodEmp & "' "
        .Source = .Source & "AND a.pdoano='" & gsAnoAct & "' "
        .Source = .Source & "AND Left(a.CodFjo, 2)='" & dcCodEli & "'"
         .Open
      End With
      If uorstCOFjo.RecordCount > 1 Then
         MsgBox Choose(gsIdioma, "Existen Flujos Relacionados, No podra Eliminar", "Exist Relationed Flows, you could not eliminate them."), vbExclamation
         With uorstCOFjo
            .Close
            .Source = "SELECT a.CodFjo, a.DetFjo, a.DetFjox "
            .Source = .Source & "FROM COFjo a "
...
Public Sub cmdImprimir_Click(Index As Integer)
 ...
 '[Datos del formulario de impresión.  'Cambiar.
   frmLFjo.Caption = Choose(gsIdioma, "Listado de ", "Listing of ") & Me.Caption
   frmLFjo.Show vbModal
 ']
End Sub
...
'            .Item(dnNum).Width = 600
             .Item(dnNum).Width = 100 * (uorstMain.Fields("CodFjo").DefinedSize + 4)
         Case 1
            .Item(dnNum).Caption = Choose(gsIdioma, "Descripción", "Description")
'            .Item(dnNum).Width = 1500
             .Item(dnNum).Width = 100 * (uorstMain.Fields("DetFjo").DefinedSize)
         Case Else
