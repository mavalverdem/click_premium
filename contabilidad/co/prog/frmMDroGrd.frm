...
'      .Properties("Unique Table").Value = "a"
   ...
   With uorstCODro
      .ActiveConnection = uocnnMain
      .Source = "SELECT a.CodDro, a.DetDro, a.DetDrox "
      .Source = .Source & "FROM CODro a "
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmMDro             'Cambiar Formulario de Datos.
End Sub
Public Sub cmdRevisar_click()
   ...
   If uorstMain.RecordCount = 0 Then
      ...
      Exit Sub
   End If
   With frmMDro                        'Cambiar Formulario de Datos.
      .zbNuevo = False
...
Dim dcCodEli As String
   ...
   End If
   '[No Pertenece al Formulario - Agregado por Angel
   dcCodEli = frmMDroGrd.uorstMain!coddro
   If Len(Trim(dcCodEli)) = 2 Then
      With uorstCODro
         ...
         .Source = .Source & "WHERE codemp='" & gsCodEmp & "' "
         .Source = .Source & "AND pdoano='" & gsAnoAct & "' "
         .Source = .Source & "AND Left(a.CodDro, 2)='" & dcCodEli & "'"
         .Open
      End With
...
'            .Item(dnNum).Width = 600
             .Item(dnNum).Width = 100 * (uorstMain.Fields("CodDro").DefinedSize + 4)
         Case 1
            .Item(dnNum).Caption = Choose(gsIdioma, "Descripción", "Description")
'            .Item(dnNum).Width = 1500
             .Item(dnNum).Width = 100 * (uorstMain.Fields("DetDro").DefinedSize)
         Case Else
