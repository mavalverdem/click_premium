...
Private Sub cmdAceptar_Click()
  ...
  With frmTCpr                     'Cambiar Formulario de Grid.
    For nContador = 0 To 2
     .txtDato(38 + nContador).Text = Format(txtDato(nContador).Text, FORMATO_NUM_1)
     .txtDato(41 + nContador).Text = Format(txtDato(3 + nContador).Text, FORMATO_NUM_1)
    ...
    .txtDato(17).Text = Format(nImporteME, FORMATO_NUM_1)
  End With
  ']
  Unload Me
...
Private Sub Form_Load()
   ...
   Dim nContador As Integer
   Me.KeyPreview = True
   Me.Caption = "Distribución del IGV Compras"
   '[Datos                            'Cambiar.
   For nContador = 0 To 5
    txtDato(nContador).MaxLength = frmTCpr.txtDato(38 + nContador).MaxLength
    txtDato(nContador).Text = frmTCpr.txtDato(38 + nContador).Text
   Next nContador
   ']
End Sub
...
Private Sub txtDato_LostFocus(Index As Integer) 'Cambiar.
   ...
   Case 0, 1, 2
    If frmTCpr.chkMonedaActiva.Value = vbChecked And frmTCpr.cboTpoMon.ListIndex = TPOMON_NAC_IND Then
       txtDato(Index + 3).Text = Format(gfRedond(CDec(txtDato(Index).Text) / CDec(frmTCpr.txtDato(4).Text), 2), FORMATO_NUM_1)
    End If
   Case 3, 4, 5
