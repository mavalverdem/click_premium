    ...
    If Len(Trim(txtDato(Index).Text)) <> 0 Then
      ...
      With frmTCprGrd.uorstCOCprDocCCo
         If Not (.BOF Or .EOF) And .RecordCount > 0 Then
            ...
            .MoveFirst
            .Find "cLlave2='" & frmTCpr.txtLlave(0).Text & frmTCpr.txtLlave(1).Text & frmTCpr.txtLlave(2).Text & frmTCpr.txtLlave(3).Text & frmTCprMasGrd.unIndice & frmTCprMasGrd.dgrMain.Columns(0).Text & txtDato(0).Text & "'"
            If Not .EOF Then
...
Private Sub ppAyuBus(tnIndex As Integer)
   Select Case tnIndex
   Case 0                              'Cambiar (añadir índices).
      modAyuBus.CCo_Cod "Length(CodCCo)=5", txtDato(tnIndex).Text, 0, 0, Me.Top + txtDato(tnIndex).Top + txtDato(tnIndex).Height, Me.Left + txtDato(tnIndex).Left
