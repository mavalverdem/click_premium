...
'   psConnStrgSele = "SELECT a.CodCCo, a.DetCCo," _
                  ...
                  & "FROM COCCo as a "
   psConnStrgSele = "SELECT COPsp.OrdRep, COPsp.CodCta, COCta.DetCta, COPsp.ImpMN_" & gsMesAct & ", " _
                  & "  COPsp.ImpME_" & gsMesAct & ", COPsp.UsrCre, COPsp.FyHCre, COPsp.UsrMdf, COPsp.FyHMdf " _
                  & "FROM COPsp Left Join COCta As COCta On COPsp.CodCta = COCta.CodCta "
...
'     .CursorLocation = adUseClient   'Es el Default.
   ...
   With porstCOCta
      .ActiveConnection = uocnnMain
      .Source = "Select CodCta, DetCta, TpoCta From COCta"
      .CursorType = adOpenDynamic
...
'      .Properties("Unique Table").Value = "COCta"
   ...
   With porstUltOrdRep
      .ActiveConnection = uocnnMain
      .Source = "SELECT IFNULL(MAX(OrdRep), 0) AS cUltOrd " _
              & "FROM COPsp "
      .CursorType = adOpenDynamic
      .LockType = adLockReadOnly
   End With
...
Public Sub cmdImprimir_Click()
 '[Datos del formulario de impresión.  'Cambiar.
   frmLPsp.Caption = "Listado de " & Me.Caption
