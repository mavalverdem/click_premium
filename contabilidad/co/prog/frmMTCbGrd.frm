...
Private Sub Form_Load()
 '[Recordsets                          'Cambiar.
   psConnStrgSele = "SELECT FehTCb, ImpTCb_Cpr, ImpTCb_Vta , "
   psConnStrgSele = psConnStrgSele & "codemp, "
   psConnStrgSele = psConnStrgSele & "UsrCre, FyHCre, UsrMdf, FyHMdf "
   psConnStrgSele = psConnStrgSele & "FROM TGTCb "
   psConnStrgSele = psConnStrgSele & "WHERE codemp='" & gsCodEmp & "' "
   psConnStrgOrde = "ORDER BY 1 DESC"
...
'     .CursorLocation = adUseClient   'Es el Default.
      ...
      .Open
      .Properties("Unique Table").Value = "TGTCb"
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmMTCb             'Cambiar Formulario de Datos.
End Sub
...
Public Sub cmdRevisar_click()
   ...
   With frmMTCb                      'Cambiar Formulario de Datos.
      .zbNuevo = False
...
Public Sub cmdEliminar_Click()
  ...
  If uorstMain.RecordCount = 0 Then
    MsgBox Choose(gsIdioma, "No hay datos creados.", "There are not created data"), vbCritical: Exit Sub
  End If
  sExpresion = Choose(gsIdioma, "Replicado en las Empresas", "Replicated in Business")
  sExpresion = TEXT_1021 & " " & Trim(dgrMain.Columns(0)) & " (" & Trim(dgrMain.Columns(1)) & ")? " & IIf(gsNvlUsr = NvlUsr_Adm, sExpresion, "")
  
  'Mensaje de verificación            'Cambiar.
  If MsgBox(sExpresion, vbYesNo + vbQuestion + vbDefaultButton2, Me.Caption) = vbYes Then
    uocnnMain.BeginTrans
    ' Senetencia de eliminacion masiva
    sExpresion = "DELETE QUICK FROM tgtcb "
    sExpresion = sExpresion & "WHERE DATE_FORMAT(fehtcb, '%Y-%m-%d')='" & Format(Trim(dgrMain.Columns(0)), "yyyy-mm-dd") & "' "
    sExpresion = sExpresion & "AND codemp IN(SELECT DISTINCT pms.codemp "
    sExpresion = sExpresion & "FROM siscfg.sgpms pms WHERE pms.codusr='" & gsCodUsr & "' "
    sExpresion = sExpresion & "AND pms.codsis='" & gsCodSis & "' AND pms.codmdl='frmMTCbGrd')"
    
    uorstMain.Delete
    If gsNvlUsr = NvlUsr_Adm Then
...
Public Sub cmdImprimir_Click(Index As Integer)
 ...
 '[Datos del formulario de impresión.  'Cambiar.
   frmLTCb.Caption = Choose(gsIdioma, "Listado de ", "Listing of ") & Me.Caption
   frmLTCb.Show vbModal
 ']
End Sub
...
If dgrMain.row <> -1 Then
   With dgrMain.Columns
      For dnNum = 0 To .Count - 1
         ...
         Case 1
            .Item(dnNum).Caption = Choose(gsIdioma, "Compra", "Purchase")
            .Item(dnNum).Width = 1300
            .Item(dnNum).Alignment = dbgRight                         'Alinear a la Dereha
            .Item(dnNum).NumberFormat = Format("#0.0000 ")    'Dar Formato a la Celda (3 Decimales)
            
         Case 2
            .Item(dnNum).Caption = Choose(gsIdioma, "Venta", "Sale")
            .Item(dnNum).Width = 1300
            .Item(dnNum).Alignment = dbgRight
            .Item(dnNum).NumberFormat = Format("#0.0000 ")
         Case Else
