...
Public uorstTGBCO As ADODB.Recordset
...
Private Sub Form_Load()
 '[Recordsets                          'Cambiar.
   ...
   psConnStrgSele = "SELECT "
   psConnStrgSele = psConnStrgSele & "codbco, tpomon, nroctacte, tpocta, codemp, codaux, "
   psConnStrgSele = psConnStrgSele & "(CASE tpocta WHEN '0' THEN 'CTE' WHEN '1' THEN 'AHO' ELSE 'MAE' END) AS sdestpocta, "
   psConnStrgSele = psConnStrgSele & "nrocci, usrcre, fyhcre, usrmdf, fyhmdf "
   psConnStrgSele = psConnStrgSele & "FROM coctaban "
   psConnStrgSele = psConnStrgSele & "WHERE coctaban.codemp='" & gsCodEmp & "' AND coctaban.codaux='" & CtaAuxiliar & "' "
   psConnStrgOrde = "ORDER BY 1"

   Set uocnnMain = New ADODB.Connection
   Set uorstMain = New ADODB.Recordset
   Set uorstTGBCO = New ADODB.Recordset
   With uocnnMain
...
'     .CursorLocation = adUseClient   'Es el Default.
      ...
      .Open
      .Properties("Unique Table").Value = "coctaban"
   End With
   With uorstTGBCO
      .ActiveConnection = uocnnMain
      .Source = "SELECT a.Codbco, a.detbco "
      .Source = .Source & "FROM COBCO a where codemp='" & gsCodEmp & "'"
'     .CursorLocation = adUseClient   'Es el Default.
...
']
  ...
  ']
   ...
   frmMCbaGrd.Caption = "Cuenta Auxiliar : " & CtaAuxiliar & " - " & DesAuxiliar
End Sub
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmMCba             'Cambiar Formulario de Datos.
End Sub
...
Public Sub cmdRevisar_click()
   ...
   With frmMCba                       'Cambiar Formulario de Datos.
      .zbNuevo = False
...
Public Sub cmdEliminar_Click()
   ...
   If MsgBox(TEXT_1021 & " " & Trim(dgrMain.Columns(0)) & " (" & Trim(dgrMain.Columns(1)) & ")?", vbYesNo + vbQuestion + vbDefaultButton2, Me.Caption) = vbYes Then
      ...
      Set Rstdelete = New ADODB.Recordset
      
      sql = "delete from coctaban "
      sql = sql & " where codemp='" & gsCodEmp & "' "
      sql = sql & " and codaux='" & CtaAuxiliar & "' "
      sql = sql & " and codbco='" & Trim(dgrMain.Columns(0)) & "'"
      sql = sql & " and tpomon='" & Trim(dgrMain.Columns(1)) & "'"
     
      Rstdelete.Open sql, uocnnMain, adOpenStatic, adLockOptimistic
...
Public Sub cmdImprimir_Click(Index As Integer)
 ...
 '[Datos del formulario de impresión.  'Cambiar.
   'frmLCba.Caption = Choose(gsIdioma, "Listado de ", "Listing of ") & Me.Caption
   'frmLCba.Show vbModal
 ']
End Sub
...
Public Sub ppDatosGrid()               'Cambiar Datos Grid.
   ...
   With dgrMain.Columns
      For dnNum = 0 To .Count - 1
         Select Case dnNum
         Case 0
            .Item(dnNum).Caption = Choose(gsIdioma, "Banco", "Bank")
            .Item(dnNum).Width = 2000
         Case 1
            .Item(dnNum).Caption = Choose(gsIdioma, "Moneda", "Money")
            .Item(dnNum).Width = 750
         Case 2
            .Item(dnNum).Caption = Choose(gsIdioma, "Cuenta Corriente", "Current Account ")
            .Item(dnNum).Width = 2000
         Case 6
            .Item(dnNum).Caption = Choose(gsIdioma, "Tipo de Cuenta", "Account Type ")
            .Item(dnNum).Width = 1230
         Case 7
            .Item(dnNum).Caption = Choose(gsIdioma, "Cuenta Interbancario", "Cuenta Interbancario")
            .Item(dnNum).Width = 2000
        Case Else
