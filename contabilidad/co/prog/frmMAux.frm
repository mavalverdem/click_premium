...
Public uorstOnp As ADODB.Recordset '2014-05-01 RR.HH afecto afp/onp
...
Public Sub puorstOnp_Insert()
'ini 2014-08-05 RR.HH afecto afp/onp
    Dim uorstCodOnpAfp As ADODB.Recordset
    Set uorstCodOnpAfp = New ADODB.Recordset
    With uorstCodOnpAfp
      .ActiveConnection = frmMAuxGrd.uocnnMain
      '.Source = "SELECT a.Codafp, " & Choose(gsIdioma, "a.DetCta", "a.DetCtax") & " AS DetCta, a.TpoTCb, a.IndDoc, a.IndCCo, a.codcco_def "
      .Source = "SELECT a.Codafp, numeroafp, a.Flagcomision, a.Fecnacimiento "
      .Source = .Source & "FROM CodOnpAfp a "
      .Source = .Source & "WHERE a.codemp='" & gsCodEmp & "' "
      .Source = .Source & "AND a.CodAux='" & txtLlave(0) & "' "
...
'      .Source = .Source & "AND a.EstCta='" & ESTCTA_ACT & "'"
      ...
      .Open
    End With
'Dim ensayo As New ADODB.Recordset
'Dim xNombre, xApellido, xedad As Variant
    Set uorstOnp = New ADODB.Recordset
    With uorstOnp
        .CursorType = adOpenStatic 'adOpenDynamic
        .Fields.Append "CodAfp", adVarChar, 2, adFldIsNullable
        .Fields.Append "NumeroAfp", adVarChar, 15, adFldIsNullable
        .Fields.Append "FlagComision", adVarChar, 1, adFldIsNullable
        .Fields.Append "FecNacimiento", adDate, 8, adFldIsNullable
        '.Fields.Append "edad", adDouble, 2, adFldIsNullable
        .Open
    End With
    With uorstCodOnpAfp
        If Not .EOF Then
            uorstOnp.AddNew
            uorstOnp.Fields("CodAfp") = !CodAfp
            uorstOnp.Fields("NumeroAfp") = !numeroafp
            uorstOnp.Fields("FlagComision") = !Flagcomision
            uorstOnp.Fields("FecNacimiento") = !Fecnacimiento
            uorstOnp.Update
        End If
    End With

'ensayo.AddNew
'ensayo.Fields("nombre") = "pepito"
'ensayo.Fields("apellido") = "perez"
'ensayo.Fields("edad") = "22"
'ensayo.Update
...
'ini 2014-08-05 RR.HH afecto afp/onp
        ...
        If Not .EOF Then
            ...
            "WHERE codemp='" & gsCodEmp & "' AND Codaux='" & txtLlave(0).Text & "'"
            '"' AND Codafp='" & .Fields("CodAfp") & "'"
        
            'If pbNuevo Then
            If Len(Trim(.Fields("CodAfp"))) <> 0 Then
                xSql = xSql & " INSERT INTO codonpafp ("
                xSql = xSql & "codemp, "
                xSql = xSql & "Codaux, "
                xSql = xSql & "Codafp, "
                xSql = xSql & "numeroafp, "
                xSql = xSql & "Flagcomision, "
                xSql = xSql & "Fecnacimiento "
                xSql = xSql & " ) VALUES ("
                xSql = xSql & "'" & gsCodEmp & "', "
                xSql = xSql & "'" & txtLlave(0).Text & "', "
                xSql = xSql & "'" & .Fields("CodAfp") & "', "
                xSql = xSql & "'" & .Fields("NumeroAfp") & "', "
                xSql = xSql & "'" & .Fields("FlagComision") & "', "
                'xSql = xSql & "'" & .Fields("FecNacimiento") & "' "
                xSql = xSql & "'" & Format(.Fields("FecNacimiento"), s_FmtFeMysql_0) & "' "
                xSql = xSql & " )"
                'sExpresion = sExpresion & "'" & gsAbvUsr & "' AS usrcre, '" & Format(Now, s_FmtFeHoMysql_0) & "' AS fyhcre "
                frmMAuxGrd.uocnnMain.Execute xSql
            End If
            'Else
            'End If
        End If
...
'   End Select
End Function
Private Sub ppSunatRuc(ByVal sNumeroRuc As String)
  Dim sCapchaAleatoria As String, sDataPage As String
  Dim sPatron As String, sElemento As String
  ...
  xWml.Open "POST", "http://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/captcha?accion=random", False
  'xWml.Open "POST", "http://www.sunat.gob.pe/cl-ti-itmrconsruc/captcha?accion=random", False
  xWml.send
  sCapchaAleatoria = xWml.responseText
  
  xWml.Open "POST", "http://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/jcrS00Alias?nroRuc=" & sNumeroRuc & "&accion=consPorRuc&numRnd=" & sCapchaAleatoria, False
  'xWml.Open "POST", "http://www.sunat.gob.pe/cl-ti-itmrconsruc/jcrS00Alias?nroRuc=" & sNumeroRuc & "&accion=consPorRuc&numRnd=" & sCapchaAleatoria, False
  xWml.send
  If xWml.Status = 200 Then
    sDataPage = xWml.responseText
    If Len(sDataPage) <= 635 Then
      MsgBox Choose(gsIdioma, "El numero Ruc ingresado no existe en la Base de datos de la SUNAT", "The entered Ruc number does not exist in the SUNAT Database"), vbInformation
      Set xWml = Nothing
      txtLlave(0).SetFocus
      Exit Sub
    End If
    
    sDataPage = Replace(sDataPage, Chr(34), "")
    sDataPage = Replace(sDataPage, Chr(13), "")
    sDataPage = Replace(sDataPage, "    ", " ")
    sDataPage = Replace(sDataPage, "   ", " ")
    sDataPage = Replace(sDataPage, "  ", " ")
    sDataPage = Replace(sDataPage, "( ", "(")
    sDataPage = Replace(sDataPage, " )", ")")
    ...
    ' RazonSocial
    sPatron = "<input type=hidden name=desRuc value="
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), ">")
    sElemento = Trim(Left(CStr(aTabla(1)), nPosicion - 1))
    sElemento = Replace(sElemento, "&#209;", "Ñ")
    sElemento = Replace(sElemento, "&#xD1;", "Ñ")
    sElemento = Replace(sElemento, "&#193;", "Á")
    sElemento = Replace(sElemento, "&#201;", "É")
    sElemento = Replace(sElemento, "&#205;", "Í")
    sElemento = Replace(sElemento, "&#211;", "Ó")
    sElemento = Replace(sElemento, "&#218;", "Ú")
    sElemento = Replace(sElemento, "&#xC1;", "Á")
    sElemento = Replace(sElemento, "&#xC9;", "É")
    sElemento = Replace(sElemento, "&#xCD;", "Í")
    sElemento = Replace(sElemento, "&#xD3;", "Ó")
    sElemento = Replace(sElemento, "&#xDA;", "Ú")
    txtDato(0).Text = sElemento
    txtDato(2).Text = sNumeroRuc
    cmbDocIdentidad(0).ListIndex = 3
    
    ' Dirección
    sPatron = "<td class=bgn colspan=1>Direcci&oacute;n del Domicilio Fiscal:</td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=3>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    sElemento = Trim(Left(sElemento, nPosicion - 1))
    sElemento = Replace(sElemento, "&#209;", "Ñ")
    sElemento = Replace(sElemento, "&#xD1;", "Ñ")
    sElemento = Replace(sElemento, "&#193;", "Á")
    sElemento = Replace(sElemento, "&#201;", "É")
    ...
    sElemento = Replace(sElemento, "&#xD3;", "Ó")
    sElemento = Replace(sElemento, "&#xDA;", "Ú")
    txtDato(1).Text = sElemento
    
    ' Telefono
    sPatron = "<td class=bgn colspan=1>Tel&eacute;fono(s):</td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=1>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    sElemento = Trim(Left(sElemento, nPosicion - 1))

    ' Nombre Comercial
    sPatron = "Nombre Comercial: </td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=1>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    ' Inscripcion
    sPatron = "<td class=bgn colspan=1>Fecha de Inscripci&oacute;n: </td"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=1>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    ' Estado
    sPatron = "<td class=bgn colspan=1>Estado del Contribuyente: </td"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=1>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    ' Sistema Emision
    sPatron = "<td class=bgn colspan=1>Sistema de Emisi&oacute;n de Comprobante:</td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=1>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    ' Actividad Exterior
    sPatron = "<td class=bgn colspan=1>Actividad de Comercio Exterior:</td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=1>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    sPatron = "<td class=bgn colspan=1>Sistema de Contabilidad:</td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=3>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    ' Tipo Contribuyente
    sPatron = "<td class=bgn colspan=1>Tipo Contribuyente: </td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=3>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    ' Emision Electronica
    sPatron = "<td class=bgn colspan=1>Emisor electr&oacute;nico desde: </td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=3>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    ' PLE
    sPatron = "<td class=bgn colspan=1>Afiliado al PLE desde: </td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=3>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    ' Actividad Economica
    sPatron = "<td class=bgn colspan=1>Actividad(es) Econ&oacute;mica(s):</td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=3>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    ' Oficio
    sPatron = "<td class=bgn colspan=1>Profesi&oacute;n u Oficio:</td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(1)), "<td class=bg colspan=1>")
    sElemento = Trim(Mid(CStr(aTabla(1)), nPosicion + 23))
    nPosicion = InStr(1, sElemento, "</td>")
    ...
    sPatron = "<td class=bgn colspan=1>Actividad(es) Econ&oacute;mica(s):</td>"
    aTabla = Split(sDataPage, sPatron)
    nPosicion = InStr(1, CStr(aTabla(2)), "<option value=00 >")
    sElemento = Trim(Mid(CStr(aTabla(2)), nPosicion + 18))
    nPosicion = InStr(1, sElemento, "</option>")
    sElemento = Trim(Left(sElemento, nPosicion - 1))
  Else
    MsgBox Choose(gsIdioma, "No responde el servicio de la SUNAT", "The SUNAT service does not respond"), vbInformation
  End If
  Set xWml = Nothing
...
Function VerificadorRUC()
  Dim lnSuma As Long, lnResiduo As Long
  Dim lnResta As Long, lnDigitoVerificador As Long
