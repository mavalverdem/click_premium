...
Public uorstCoEntidadPen As ADODB.Recordset
Public uorstCoCta As ADODB.Recordset
']

Private Sub Form_Load()
 '[Recordsets                          'Cambiar.
   psConnStrgSele = "SELECT Codafp, "
   psConnStrgSele = psConnStrgSele & "Desafp, CodSunat, "
   psConnStrgSele = psConnStrgSele & "Factor1, Factor2, "
   psConnStrgSele = psConnStrgSele & "Factor3, Factor4, "
   psConnStrgSele = psConnStrgSele & "topeseg, Estadoafp, "
   psConnStrgSele = psConnStrgSele & "codemp,  UsrCre, FyHCre, UsrMdf, FyHMdf,codcta "
   psConnStrgSele = psConnStrgSele & "FROM CoEntidadPen "
...
'     .CursorLocation = adUseClient   'Es el Default.
      ...
      .Open
      .Properties("Unique Table").Value = "CoEntidadPen"
'      .Properties("Unique Table").Value = "a"
   End With
   With uorstCoEntidadPen
      .ActiveConnection = uocnnMain
      '*.Source = "SELECT a.Codafp, a.Desafp, a.Desafpx "
      .Source = "SELECT a.Codafp, a.Desafp, a.Codsunat "
...
'     .CursorLocation = adUseClient   'Es el Default.
   ...
   With uorstCoCta
      .ActiveConnection = uocnnMain
      .Source = "SELECT a.codcta, a.DetCta "
 ...
 ']
  ...
  ']
   Me.Caption = Choose(gsIdioma, "Mantenimiento de ONP/AFP", "Maintenance ONP/AFP")
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmMOnp             'Cambiar Formulario de Datos.
End Sub
Public Sub cmdRevisar_click()
   ...
   If uorstMain.RecordCount = 0 Then
      ...
      Exit Sub
   End If
   With frmMOnp                        'Cambiar Formulario de Datos.
      .zbNuevo = False
...
'            .Item(dnNum).Width = 600
             .Item(dnNum).Width = 100 * (uorstMain.Fields("Codafp").DefinedSize + 4)
         Case 1
            .Item(dnNum).Caption = Choose(gsIdioma, "Descripción", "Description")
'            .Item(dnNum).Width = 1500
             .Item(dnNum).Width = 100 * (uorstMain.Fields("Desafp").DefinedSize)
         Case Else
