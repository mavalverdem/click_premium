...
Private Sub Form_Load()
 '[Recordsets                          'Cambiar.
   psConnStrgSele = "SELECT CodCCo, "
   psConnStrgSele = psConnStrgSele & Choose(gsIdioma, "DetCCo, ", "DetCCox, ")
   psConnStrgSele = psConnStrgSele & "(CASE EstCCo WHEN '" & ESTCCO_ACT & "' THEN '" & ESTCCO_ACT_TXT & "' ELSE '" & ESTCCO_INA_TXT & "' END) AS cEstCCo, "
   psConnStrgSele = psConnStrgSele & Choose(gsIdioma, "DetCCox, ", "DetCCo, ") & "indpdocpr, EstCCo, "
   psConnStrgSele = psConnStrgSele & "UsrCre, FyHCre, UsrMdf, FyHMdf, "
   psConnStrgSele = psConnStrgSele & "codemp, pdoano "
   psConnStrgSele = psConnStrgSele & "FROM COCCo "
...
'      .Properties("Unique Table").Value = "COCCo"
   ...
   With uorstCodCCo
      .ActiveConnection = uocnnMain
      .Source = "SELECT a.CodCCo, a.DetCCo, a.DetCCox "
...
Public Sub cmdNuevo_Click()
   gpTUg_Nuevo Me, frmMCCo             'Cambiar Formulario de Datos.
End Sub
...
Public Sub cmdRevisar_click()
   ...
   With frmMCCo                        'Cambiar Formulario de Datos.
      .zbNuevo = False
...
Public Sub cmdEliminar_Click()
 ...
 '[No Pertenece al Formulario - Agregado por Angel
   dcCodEli = frmMCCoGrd.uorstMain!codcco
   If Len(Trim(dcCodEli)) = 2 Then
      With uorstCodCCo
         ...
         .Source = .Source & "AND Left(a.CodCCo, 2)='" & dcCodEli & "'"
         .Open
      End With
      If uorstCodCCo.RecordCount > 1 Then
         MsgBox Choose(gsIdioma, "Existen Centros de Costos Relacionados, No podra Eliminar", "Exist Relationed Cost Center, Can not eliminate"), vbExclamation
         With uorstCodCCo
            .Close
            .Source = "SELECT a.CodCCo, a.DetCCo, a.DetCCox "
            .Source = .Source & "FROM COCCo a "
'ini 2016-03-31 error centro de costo
            '.Source = .Source & "WHERE a.codemp=" & gsCodEmp & "' "
            '.Source = .Source & "AND a.pdoano=" & gsAnoAct & "'"
            .Source = .Source & "WHERE a.codemp='" & gsCodEmp & "' "
            .Source = .Source & "AND a.pdoano='" & gsAnoAct & "'"
'fin 2016-03-31 error centro de costo
            .Open
         End With
         dgrMain.SetFocus
         Exit Sub
...
Public Sub ppDatosGrid()               'Cambiar Datos Grid.
   ...
   With dgrMain.Columns
      For dnNum = 0 To .Count - 1
         ...
         Case 2
            .Item(dnNum).Caption = Choose(gsIdioma, "Situación", "Situation")
