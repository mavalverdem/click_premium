...
Public Sub Pdo_Sal(psWhere As String, pvDato1Previo As Variant, pnAlto, pnAncho, pnArriba As Integer, pnIzquierda As Integer)
  ...
  End With
End Sub
Public Sub Prod_Cod(psWhere As String, pvDato1Previo As Variant, pnAlto, pnAncho, pnArriba As Integer, pnIzquierda As Integer)
  With frmOAyuBus
    .usConnStrgSele = "SELECT codprod, " & Choose(gsIdioma, "detprod", "detprodx") & " AS detprod "
    .usConnStrgSele = .usConnStrgSele & "FROM cocprprod "
    .usConnStrgSele = .usConnStrgSele & "WHERE codemp='" & gsCodEmp & "' AND pdoano='" & gsAnoAct & "' "
    .usConnStrgSele = .usConnStrgSele & IIf(psWhere = "", "", "AND " & psWhere & " ")
    .usConnStrgOrde = "ORDER BY 1"
    .uaTitulos = Array(Choose(gsIdioma, "Codigo", "Code"), Choose(gsIdioma, "Descripción", "Description"))
    .uaAncho = Array(1600, 5500)
    .uaAlineamiento = Array(dbgGeneral, dbgGeneral)
    .uaFormato = Array("", "")
    .uaOrden = Array("", "")
    .uvDato1Previo = pvDato1Previo
    .usCriterio = "codprod='" & pvDato1Previo & "'"
...
Public Sub Con_Rpt(psWhere As String, pvDato1Previo As Variant, pnAlto, pnAncho, pnArriba As Integer, pnIzquierda As Integer)
   With frmOAyuBus
      .usConnStrgSele = "SELECT a.codcon, " & Choose(gsIdioma, "a.detcon", "a.detconx") & " AS detcon "
      .usConnStrgSele = .usConnStrgSele & "FROM coconser a "
      .usConnStrgSele = .usConnStrgSele & "WHERE a.codemp='" & gsCodEmp & "' "
      .usConnStrgSele = .usConnStrgSele & IIf(psWhere = "", "", "AND " & psWhere & " ")
      .usConnStrgOrde = "ORDER BY codcon "
      .uaTitulos = Array(Choose(gsIdioma, "Orden Servicio", "Service Order"), Choose(gsIdioma, "Detalle", "Detail"))
      .uaAncho = Array(1200, 3000)
      .uaAlineamiento = Array(dbgGeneral, dbgGeneral)
      .uaFormato = Array("", "")
      .uaOrden = Array("", "")
      .uvDato1Previo = pvDato1Previo
      .usCriterio = "codcon='" & pvDato1Previo & "'"
...
Public Sub Con_Sal(psWhere As String, pvDato1Previo As Variant, pnAlto, pnAncho, pnArriba As Integer, pnIzquierda As Integer)
  With frmOAyuBus
    .usConnStrgSele = "SELECT a.codcon, " & Choose(gsIdioma, "a.detcon", "a.detconx") & " AS detcon, a.tpomon, "
    If ps_Plataforma = pSrvMySql Then
      .usConnStrgSele = .usConnStrgSele & "ROUND(CASE a.tpomon WHEN '" & TPOMON_NAC & "' THEN (a.impmn-"
      .usConnStrgSele = .usConnStrgSele & "IFNULL(ROUND(SUM("
      .usConnStrgSele = .usConnStrgSele & "((IFNULL(b.impogr_mn, 0)+IFNULL(b.impexp_mn, 0)+IFNULL(b.impexo_mn, 0))*"
      .usConnStrgSele = .usConnStrgSele & "(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))"
      .usConnStrgSele = .usConnStrgSele & "), 2), 0)) "
      .usConnStrgSele = .usConnStrgSele & "ELSE (a.impme-"
      .usConnStrgSele = .usConnStrgSele & "IFNULL(ROUND(SUM("
      .usConnStrgSele = .usConnStrgSele & "((IFNULL(b.impogr_me, 0)+IFNULL(b.impexp_me, 0)+IFNULL(b.impexo_me, 0))*"
      .usConnStrgSele = .usConnStrgSele & "(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))"
      .usConnStrgSele = .usConnStrgSele & "), 2), 0)) END, 2) AS cImpSaldo, "
    Else
      .usConnStrgSele = .usConnStrgSele & "ROUND(CASE a.tpomon WHEN '" & TPOMON_NAC & "' THEN (a.impmn-"
      .usConnStrgSele = .usConnStrgSele & "ISNULL(ROUND(SUM("
      .usConnStrgSele = .usConnStrgSele & "((ISNULL(b.impogr_mn, 0)+ISNULL(b.impexp_mn, 0)+ISNULL(b.impexo_mn, 0))*"
      .usConnStrgSele = .usConnStrgSele & "(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))"
      .usConnStrgSele = .usConnStrgSele & "), 2), 0)) "
      .usConnStrgSele = .usConnStrgSele & "ELSE (a.impme-"
      .usConnStrgSele = .usConnStrgSele & "ISNULL(ROUND(SUM("
      .usConnStrgSele = .usConnStrgSele & "((ISNULL(b.impogr_me, 0)+ISNULL(b.impexp_me, 0)+ISNULL(b.impexo_me, 0))*"
      .usConnStrgSele = .usConnStrgSele & "(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))"
      .usConnStrgSele = .usConnStrgSele & "), 2), 0)) END, 2) AS cImpSaldo, "
    End If
    .usConnStrgSele = .usConnStrgSele & "a.fehcon "
    .usConnStrgSele = .usConnStrgSele & "FROM ((coconser a "
    .usConnStrgSele = .usConnStrgSele & "LEFT JOIN covtadoc b ON a.codemp=b.codemp AND a.codaux=b.codaux AND a.codcon=b.codcon "
    If ps_Plataforma = pSrvMySql Then
      .usConnStrgSele = .usConnStrgSele & "AND IFNULL(b.codcon, '')<>'' "
      .usConnStrgSele = .usConnStrgSele & "AND Concat(b.pdoano, b.mespvs)<='" & gsAnoAct & gsMesAct & "' "
      .usConnStrgSele = .usConnStrgSele & "AND b.feedoc<=" & Right(Trim(psWhere), 12) & ") "
    ElseIf ps_Plataforma = pSrvSql Then
      .usConnStrgSele = .usConnStrgSele & "AND ISNULL(b.codcon, '')<>'' "
      .usConnStrgSele = .usConnStrgSele & "AND (b.pdoano+b.mespvs)<='" & gsAnoAct & gsMesAct & "' "
      .usConnStrgSele = .usConnStrgSele & "AND b.feedoc<= CONVERT(smalldatetime, " & Right(Trim(psWhere), 12) & ", 103)) "
    End If
    .usConnStrgSele = .usConnStrgSele & "LEFT JOIN TGTDc c ON b.codemp=c.codemp AND b.CodTDc=c.CodTDc) "
    ...
    .usConnStrgSele = .usConnStrgSele & "AND " & IIf(ps_Plataforma = pSrvMySql, "CONCAT(a.pdoano, a.mespvs)", "(a.pdoano+a.mespvs)") & "<='" & gsAnoAct & gsMesAct & "' "
    .usConnStrgSele = .usConnStrgSele & IIf(psWhere = "", "", "AND " & psWhere & " ")
    .usConnStrgSele = .usConnStrgSele & "GROUP BY a.codemp, a.codaux, a.codcon "
    If ps_Plataforma = pSrvMySql Then
      .usConnStrgSele = .usConnStrgSele & "HAVING cImpSaldo<>0.00 "
    Else
      .usConnStrgSele = .usConnStrgSele & "HAVING (ROUND(CASE a.tpomon WHEN '" & TPOMON_NAC & "' THEN (a.impmn-"
      .usConnStrgSele = .usConnStrgSele & "ISNULL(ROUND(SUM((ISNULL(b.impogr_mn, 0)*(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))+"
      .usConnStrgSele = .usConnStrgSele & "(ISNULL(b.impexp_mn, 0)*(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))+"
      .usConnStrgSele = .usConnStrgSele & "(ISNULL(b.impexo_mn, 0)*(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))), 2), 0)) ELSE "
      .usConnStrgSele = .usConnStrgSele & "(a.impme-ISNULL("
      .usConnStrgSele = .usConnStrgSele & "ROUND(SUM((ISNULL(b.impogr_me, 0)*(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))+"
      .usConnStrgSele = .usConnStrgSele & "(ISNULL(b.impexp_me, 0)*(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))+"
      .usConnStrgSele = .usConnStrgSele & "(ISNULL(b.impexo_me, 0)*(CASE c.SgnTDc WHEN '" & SGNTDC_NEG & "' THEN -1 ELSE 1 END))), 2), 0)) END, 2))<>0.00 "
    End If
    .usConnStrgOrde = "ORDER BY a.codcon"
    .uaTitulos = Array(Choose(gsIdioma, "Orden Servicio", "Service Order"), Choose(gsIdioma, "Detalle", "Detail"), Choose(gsIdioma, "Mon", "Cur"), Choose(gsIdioma, "Saldo", "Rest"))
    .uaAncho = Array(1100, 2000, 250, 1200)
    .uaAlineamiento = Array(dbgGeneral, dbgGeneral, dbgCenter, dbgRight)
    .uaFormato = Array("", "", "", FORMATO_NUM_1 & " ")
    .uaOrden = Array("", "", "", "")
...
Public Sub Cpb_Dro(psWhere As String, pvDato1Previo As Variant, pnAlto, pnAncho, pnArriba As Integer, pnIzquierda As Integer)
   With frmOAyuBus
      ...
      .usConnStrgSele = .usConnStrgSele & "FROM CoCpbCab " '2015-09-04 error bitacora y ctr mayoriza
...
Public Sub OnoAfp_Cod(psWhere As String, pvDato1Previo As Variant, pnAlto, pnAncho, pnArriba As Integer, pnIzquierda As Integer)
   With frmOAyuBus
      .usConnStrgSele = "SELECT Codafp, " & Choose(gsIdioma, "Desafp", "Desafp") & " AS Desafp "
      .usConnStrgSele = .usConnStrgSele & "FROM Coentidadpen "
      .usConnStrgSele = .usConnStrgSele & "WHERE codemp='" & gsCodEmp & "' "
      '.usConnStrgSele = .usConnStrgSele & "AND pdoano='" & gsAnoAct & "' "
      .usConnStrgSele = .usConnStrgSele & IIf(psWhere = "", "", "AND " & psWhere & " ")
      .usConnStrgOrde = "ORDER BY 1"
      ...
      .uaOrden = Array("", "")
      .uvDato1Previo = pvDato1Previo
      .usCriterio = "Codafp='" & pvDato1Previo & "'"
...
Public Sub Detrac_Cod(psWhere As String, pvDato1Previo As Variant, pnAlto, pnAncho, pnArriba As Integer, pnIzquierda As Integer)
   With frmOAyuBus
      '.ubBDConfiguracion = False     'Para BD de Configuración.
   
      .usConnStrgSele = "SELECT coddetrac, " & Choose(gsIdioma, "detdetrac", "detdetracx") & " AS detdetrac "
      .usConnStrgSele = .usConnStrgSele & "FROM codetrac "
      .usConnStrgSele = .usConnStrgSele & "WHERE codemp='" & gsCodEmp & "' "
      '2015-06-26 proceso detrac  .usConnStrgSele = .usConnStrgSele & "AND pdoano='" & gsAnoAct & "' "
      .usConnStrgSele = .usConnStrgSele & IIf(psWhere = "", "", "AND " & psWhere & " ")
      .usConnStrgOrde = "ORDER BY 1"
      .uaTitulos = Array(Choose(gsIdioma, "Código", "Code"), Choose(gsIdioma, "Descripción", "Description"))
      .uaAncho = Array(800, 6500)
      ...
      .uaOrden = Array("", "")
      .uvDato1Previo = pvDato1Previo
      .usCriterio = "coddetrac='" & pvDato1Previo & "'"
...
Public Sub Sunat_Cod(psWhere As String, pvDato1Previo As Variant, pnAlto, pnAncho, pnArriba As Integer, pnIzquierda As Integer, pnCodTabla As String)
   With frmOAyuBus
         .ubBDConfiguracion = True     'Para BD de Configuración.

        If pnCodTabla = CODSUNAT_004 Then
          .usConnStrgSele = "SELECT codsunat, campo03, detsunat "
        Else
          .usConnStrgSele = "SELECT codsunat, detsunat, campo03 "
        End If
      .usConnStrgSele = .usConnStrgSele & "FROM tgsunat "
      .usConnStrgSele = .usConnStrgSele & "WHERE codtabla='" & pnCodTabla & "' "
      .usConnStrgSele = .usConnStrgSele & IIf(psWhere = "", "", "AND " & psWhere & " ")
      .usConnStrgOrde = "ORDER BY 1"
      .uaTitulos = Array(Choose(gsIdioma, "Código", "Code"), Choose(gsIdioma, "Descripción", "Description"), Choose(gsIdioma, "Descripción 2 ", "Description 2"))
      .uaAncho = Array(800, 4500, 800)
      .uaAlineamiento = Array(dbgGeneral, dbgGeneral, dbgGeneral)
      .uaFormato = Array("", "", "")
      .uaOrden = Array("", "", "")
      .uvDato1Previo = pvDato1Previo
      .usCriterio = "codsunat='" & pvDato1Previo & "'"
