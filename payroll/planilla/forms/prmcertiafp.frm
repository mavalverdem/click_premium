...
Begin VB.Form fPrmCertifikAfp 
   BorderStyle     =   1  'Fixed Single
   ClientHeight    =   4020
   ClientLeft      =   2265
   ClientTop       =   375
   ClientWidth     =   6420
   Icon            =   "prmcertiafp.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ScaleHeight     =   4020
   ...
   Begin TabDlg.SSTab tabRegister 
      ...
      TabPicture(0)   =   "prmcertiafp.frx":000C
      ...
      Tab(0).Control(8)=   "cmdHelp(1)"
      Tab(0).Control(8).Enabled=   0   'False
      Tab(0).Control(9)=   "cmdHelp(3)"
      Tab(0).Control(9).Enabled=   0   'False
      Tab(0).Control(10)=   "cmdHelp(2)"
      Tab(0).Control(10).Enabled=   0   'False
      Tab(0).Control(11)=   "cmdHelp(0)"
      Tab(0).Control(11).Enabled=   0   'False
      Tab(0).Control(12)=   "txtRemunera"
      Tab(0).Control(12).Enabled=   0   'False
      Tab(0).Control(13)=   "txtRetencion(1)"
      Tab(0).Control(13).Enabled=   0   'False
      Tab(0).Control(14)=   "txtRetencion(2)"
      Tab(0).Control(14).Enabled=   0   'False
      Tab(0).Control(15)=   "txtRetencion(0)"
   ...
   End
End
Attribute VB_Name = "fPrmCertifikAfp"
...
Sub ShowScreen()
  ...
  s_Sql = "SELECT cpcremase, cpcapobli, cpcseguro, cpcporcen "
  ...
  If Not (porstRecordset.BOF And porstRecordset.BOF) Then
    ...
    gdl_Procedure.EditText "AT", txtRemunera, gdl_Funcion.aTexto(porstRecordset!cpcremase), Me.Tag, False, porstRecordset!cpcremase.DefinedSize
    gdl_Procedure.EditText "AT", txtRetencion(0), gdl_Funcion.aTexto(porstRecordset!cpcapobli), Me.Tag, False, porstRecordset!cpcapobli.DefinedSize
    gdl_Procedure.EditText "AT", txtRetencion(1), gdl_Funcion.aTexto(porstRecordset!cpcseguro), Me.Tag, False, porstRecordset!cpcseguro.DefinedSize
    gdl_Procedure.EditText "AT", txtRetencion(2), gdl_Funcion.aTexto(porstRecordset!cpcporcen), Me.Tag, False, porstRecordset!cpcporcen.DefinedSize
  Else
    Me.Tag = s_MdoData_Ins
    gdl_Procedure.EditText "AT", txtRemunera, "", Me.Tag, False, porstRecordset!cpcremase.DefinedSize
    gdl_Procedure.EditText "AT", txtRetencion(0), "", Me.Tag, False, porstRecordset!cpcapobli.DefinedSize
    gdl_Procedure.EditText "AT", txtRetencion(1), "", Me.Tag, False, porstRecordset!cpcseguro.DefinedSize
    gdl_Procedure.EditText "AT", txtRetencion(2), "", Me.Tag, False, porstRecordset!cpcporcen.DefinedSize
  End If
  ...
  lblHelp(2) = gdl_Funcion.DameDescripcion(ps_StrgConnec & ps_DataBase, ps_CodEmpresa, txtRetencion(1), "CP")
  lblHelp(3) = gdl_Funcion.DameDescripcion(ps_StrgConnec & ps_DataBase, ps_CodEmpresa, txtRetencion(2), "CP")
...
Private Sub cmdHelp_Click(Index As Integer)
  ...
  Select Case Index
   ...
   Case 1, 2, 3   ' Conceptos de planilla de descuento
    tdbHelp.Columns(0).DataField = "codcpc": tdbHelp.Columns(1).DataField = "descpc"
  ...
  tdbHelp.Top = tabRegister.Top + (Choose(Index + 1, cmdHelp(Index).Top, 750, 800, 850) + (cmdHelp(Index).Height / 2))
...
Private Sub cmdUpdate_Click(Index As Integer)
  ...
  If txtRetencion(0) = "" Then Beep: MsgBox "Debe Ingresar el Concepto de Aporte Obligatorio", vbExclamation: txtRetencion(0).SetFocus: Exit Sub
  If lblHelp(1) = "???" Then Beep: MsgBox "Concepto de Remuneración Aporte Obligatorio no es valido; Verificar", vbExclamation: txtRetencion(0).SetFocus: Exit Sub
  If txtRetencion(1) = "" Then Beep: MsgBox "Debe Ingresar el Concepto de Seguros", vbExclamation: txtRetencion(1).SetFocus: Exit Sub
  If lblHelp(2) = "???" Then Beep: MsgBox "Concepto de Seguros no es valido; Verificar", vbExclamation: txtRetencion(1).SetFocus: Exit Sub
  If txtRetencion(2) = "" Then Beep: MsgBox "Debe Ingresar el Concepto de Comisión % sobre R.A.", vbExclamation: txtRetencion(2).SetFocus: Exit Sub
  If lblHelp(3) = "???" Then Beep: MsgBox "Concepto de Comisión % sobre R.A. no es valido; Verificar", vbExclamation: txtRetencion(2).SetFocus: Exit Sub
    
  ' Coloco el puntero en espera
  gdl_Procedure.PunteroEnEspera
    
  ' Creo los arreglos para la actualización
  a_Campos = Array("pdoano", "cpcremase", "cpcapobli", "cpcseguro", "cpcporcen", IIf(Me.Tag = s_MdoData_Ins, "usrcre", "usrmdf"), IIf(Me.Tag = s_MdoData_Ins, "fyhcre", "fyhmdf"))
  a_Valores = Array(ps_Anyo, txtRemunera, txtRetencion(0), txtRetencion(1), txtRetencion(2), ps_Usuario, Format(Now, s_FmtFeHoMysql_0))
...
Private Sub tdbHelp_DblClick()
  ...
  Select Case n_IndexHelp
   Case 0         ' Concepto de remuneración asegurable
    ...
    txtRemunera.SetFocus
   Case 1, 2, 3   ' Concepto de retencion de pensiones
    txtRetencion(n_IndexHelp - 1) = tdbHelp.Columns(0).Value
    lblHelp(n_IndexHelp) = tdbHelp.Columns(1).Value
    txtRetencion(n_IndexHelp - 1).SetFocus
  End Select
...
Private Sub tdbHelp_HeadClick(ByVal ColIndex As Integer)
  ...
  Select Case n_IndexHelp
   Case 0         ' Conceptos de remuneración asegurable
    s_Registro = ps_ClsPlanilla & "F" & s_Estado_Ina
    s_Sql = gdl_Funcion.HelpTablas("cxt", tdbHelp.Columns(ColIndex).DataField, s_Registro, "")
   Case 1, 2, 3   ' Concepto de aportes de pensiones
    s_Registro = ps_ClsPlanilla & "F" & s_Estado_Act
    s_Sql = gdl_Funcion.HelpTablas("cxt", tdbHelp.Columns(ColIndex).DataField, s_Registro, "")
  End Select
  Set porstHelp = OpenRecordset(ps_StrgConnec & ps_DataBase, adOpenForwardOnly, adLockReadOnly, adUseClient, s_Sql)
...
Private Sub txtRetencion_LostFocus(Index As Integer)
  lblHelp(Index + 1) = gdl_Funcion.DameDescripcion(ps_StrgConnec & ps_DataBase, ps_CodEmpresa, txtRetencion(Index), "CP")
End Sub
