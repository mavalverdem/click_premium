...
Begin VB.Form fCentroCosto 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         MouseIcon       =   "cencosto.frx":0028
         Picture         =   "cencosto.frx":0182
      End
      Begin Threed.SSCommand cmdAction 
         Height          =   360
         Index           =   3
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "cencosto.frx":029A
      End
   End
End
Attribute VB_Name = "fCentroCosto"
...
Public ps_NivelCCo As String                            ' Niveles del codigo de centro de costo
...
Private Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  gdl_Procedure.SeteaAdoControl ps_StrgConnec & ps_DaBasCon, dcaRegistro, tdbRegistro, s_Sql, adCmdText, adLockReadOnly
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  Select Case Index
   ...
   Case 6, 7  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = "CENTRO DE COSTOS"
    gdl_Procedure.ps_ReportName = "lstcencosto"
    ...
    ' Ejecuto el reporto
    gdl_Procedure.ParametersPrinter ps_StrgConnec & ps_DaBasCon, fMenu.CryReport, (Index - 6), False, True, False, True, True, aElemento, aElementos, porstRecordset
   ...
   Case 8
    ...
    If Not (Len(dcaRegistro.Recordset!codcco) >= CInt(Right(ps_NivelCCo, 1))) Then Beep: MsgBox "Nivel de " & s_TitleTable & " no es Actualizable", vbExclamation: Exit Sub
    fPrmCentroCosto.Show vbModal
   Case 9, 10  ' Visualiza las cuentas de planilla  y provision por centro de costo
    ' Verifico que Existan Registros
    If dcaRegistro.Recordset.RecordCount = 0 Then Beep: MsgBox "No Existen " & s_TitleTable & " para Actualizar", vbExclamation: Exit Sub
    If Not (Len(dcaRegistro.Recordset!codcco) >= CInt(Right(ps_NivelCCo, 1))) Then Beep: MsgBox "Nivel de " & s_TitleTable & " no es Actualizable", vbExclamation: Exit Sub
    If Index = 9 Then
      fCuentaCentroCosto.Show
    Else
      fCuentaProvision.Show
    End If
   Case 11    ' Copia infromación de otros centros de costo
   
   If Not (Len(dcaRegistro.Recordset!codcco) = CInt(Right(ps_NivelCCo, 1))) Then Beep: MsgBox "Nivel de " & s_TitleTable & " no es Actualizable", vbExclamation: Exit Sub
    Val = InputBox("" & vbCrLf & vbCrLf & "Copiar Datos del Centro de Costos :", "Centro de Costos", "")
    If Not (Len(Val) = CInt(Right(ps_NivelCCo, 1))) Then Beep: MsgBox "Nivel de " & s_TitleTable & " origen no Valido", vbExclamation: Exit Sub

    TipodeProgreso = 1
    IntervalodeTiempo = 100
    labelprogreso = "Generando                                                          Datos"
    'cuenta contable
    s_sql_delete = "DELETE FROM plctacencos where codcls='" & ps_ClsPlanilla & "' AND codcco='" & tdbRegistro.Columns(0).Text & "'"
    rsdelete.Open s_sql_delete, cnn, adOpenStatic, adLockOptimistic
    On Error GoTo Errores1
    s_sql_insert = "INSERT INTO plctacencos (codcls,codcco,codsec,codcpc,orden,codafp,codcta_debmn,codcta_habmn,codcta_debme,codcta_habme,usrcre,fyhcre) "
    s_sql_insert = s_sql_insert & "SELECT codcls,'" & tdbRegistro.Columns(0).Text & "',codsec,codcpc,orden,codafp,codcta_debmn,codcta_habmn,codcta_debme,codcta_habme,'admin',now() FROM plctacencos "
    s_sql_insert = s_sql_insert & "WHERE codcls='" & ps_ClsPlanilla & "' AND codcco='" & Val & "'"
    rsinsert.Open s_sql_insert, cnn, adOpenStatic, adLockOptimistic
    On Error GoTo Errores1
    'provision
    s_sql_delete = "DELETE FROM plctapvs WHERE codcls='" & ps_ClsPlanilla & "' AND codcco='" & tdbRegistro.Columns(0).Text & "'"
    rsdelete.Open s_sql_delete, cnn, adOpenStatic, adLockOptimistic
    On Error GoTo Errores1
    s_sql_insert = "INSERT INTO plctapvs (codcls,codcco,orden,codctav_debmn,codctav_habmn,codctav_debme,codctav_habme,codctag_debmn,codctag_habmn,codctag_debme,codctag_habme,usrcre,fyhcre) "
    s_sql_insert = s_sql_insert & "SELECT codcls,'" & tdbRegistro.Columns(0).Text & "',orden,codctav_debmn,codctav_habmn,codctav_debme,codctav_habme,codctag_debmn,codctag_habmn,codctag_debme,codctag_habme,'admin',now() from plctapvs "
    s_sql_insert = s_sql_insert & "WHERE codcls='" & ps_ClsPlanilla & "' AND codcco='" & Val & "'"
    rsinsert.Open s_sql_insert, cnn, adOpenStatic, adLockOptimistic
    On Error GoTo Errores1
...
Private Sub dcaRegistro_MoveComplete(ByVal adReason As ADODB.EventReasonEnum, ByVal pError As ADODB.Error, adStatus As ADODB.EventStatusEnum, ByVal pRecordset As ADODB.Recordset)
  ...
  If FormVisible("fAbcCentroCosto") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      fAbcCentroCosto.ShowScreen
    End If
  End If
  If FormVisible("fCuentaCentroCosto") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      fCuentaCentroCosto.RecuperaRegistros "codcpc"
    End If
  End If
  If FormVisible("fCuentaProvision") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      fCuentaProvision.RecuperaRegistros "orden"
    End If
  End If
...
Private Sub Form_Load()
  ...
  For i = 0 To (UBound(aElemento, 1) - 1)
    aElemento(i, 1) = Choose(i + 1, "seleccio", "anadir", "borrar", "ordascen", "orddesce", "busqueda", "prelimin", "Imprimir", "asoclibr", "analsald", "analmovs", "consolid")
    aElemento(i, 2) = Choose(i + 1, "Selecciona y Edita " & s_TitleTable, "Añadir " & s_TitleTable, "Eliminar " & s_TitleTable, "Ordenar Ascendente", "Ordenar Descendente", "Buscar " & s_TitleTable, "Presentación Preliminar", "Imprimir", "Información del Negocio", "Cuentas Contables Planillas", "Cuentas Contables Provisiones", "Copiar Datos")
  Next i
  gdl_Procedure.ViewGrafics Me, cmdAction, aElemento
  
  ' Configuración de niveles centro de costo
  ps_NivelCCo = "2"
  s_Sql = "SELECT codcco_nv3, codcco_nv5 "
  s_Sql = s_Sql & " FROM cocfg "
  Set porstRecordset = OpenRecordset(ps_StrgConnec & ps_DaBasCon, adOpenForwardOnly, adLockReadOnly, adUseClient, s_Sql)
  If Not (porstRecordset.EOF And porstRecordset.BOF) Then
'    ps_NivelCCo = ps_NivelCCo & IIf(porstRecordset!codcco_nv3, "3", "") & IIf(porstRecordset!codcco_nv5, "5", "")
  End If
  ' Cierro objeto
  porstRecordset.Close
  ps_NivelCCo = ps_NivelCCo & pn_NivelCenCosto
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcCentroCosto") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcCentroCosto.Caption, vbExclamation
    Cancel = True
  End If
  If FormVisible("fCuentaCentroCosto") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fCuentaCentroCosto.Caption, vbExclamation
    Cancel = True
  End If
  If FormVisible("fCuentaProvision") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fCuentaProvision.Caption, vbExclamation
    Cancel = True
  End If
