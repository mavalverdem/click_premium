...
Begin VB.Form fBoletaPago 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "boletapago.frx":00D0
      End
   End
End
Attribute VB_Name = "fBoletaPago"
...
Private Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT codboleta, desboleta, orientacion, calidad, papelancho,"
  s_Sql = s_Sql & " papelalto, font, copia, lininicopia, estadobol"
  s_Sql = s_Sql & " FROM plboletapago"
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  Select Case Index
   ...
   Case 6, 7  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = "FORMATO DE BOLETAS DE PAGO"
    gdl_Procedure.ps_ReportName = "lstformaboleta"
    ...
    s_Sql = "SELECT bol.codboleta, bol.desboleta, bol.orientacion, bol.calidad, bol.papelancho,"
    s_Sql = s_Sql & " bol.papelalto, bol.font, bol.copia, bol.lininicopia, fmt.seccion, fmt.origen, fmt.dato,"
    s_Sql = s_Sql & " IF(fmt.origen='C', cpc.descpc, var.descripcion) AS detalle, fmt.tipodato, fmt.fila,"
    s_Sql = s_Sql & " fmt.columna, fmt.longitud, fmt.sizefont, fmt.fontn, fmt.fonts , fmt.fontc"
    s_Sql = s_Sql & " FROM plboletapago bol"
    s_Sql = s_Sql & " INNER JOIN pldetaboleta fmt ON bol.codcls=fmt.codcls AND bol.codboleta=fmt.codboleta"
    s_Sql = s_Sql & " LEFT JOIN plvarfunc var ON fmt.origen=var.tipo AND fmt.dato=var.codigo AND var.tipo='D'"
    s_Sql = s_Sql & " LEFT JOIN plconcepto cpc ON fmt.dato=cpc.codcpc AND fmt.origen='C'"
    s_Sql = s_Sql & " WHERE bol.codcls='" & ps_ClsPlanilla & "'"
    s_Sql = s_Sql & " ORDER BY codboleta, fila, columna"
...
Private Sub Form_Load()
  ...
  s_TitleTable = "Formato de Boleta de Pago"
  
  ReDim aElemento(3, 10)
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 0) = Choose(n_Index + 1, "Código", "Descripción", "Ok")
    aElemento(n_Index, 1) = Choose(n_Index + 1, "codboleta", "desboleta", "estadobol")
  ...
  For n_Index = 0 To 1
    ...
    tdbRegistro.Columns(2).ValueItems.Item(n_Index).DisplayValue = LoadPicture(gdl_Procedure.ps_PathImagen & Choose(n_Index + 1, "estadnok", "estadok") & ".bmp")
  Next n_Index
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcFormatoBoleta") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcFormatoBoleta.Caption, vbExclamation
    Cancel = True
  End If
