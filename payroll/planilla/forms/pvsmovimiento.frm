...
Begin VB.Form fPvsMovimiento 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsmovimiento.frx":000C
      End
      Begin Threed.SSCommand cmdAction 
         Height          =   360
         Index           =   2
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsmovimiento.frx":0028
      End
      Begin Threed.SSCommand cmdAction 
         Height          =   360
         Index           =   3
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsmovimiento.frx":0044
      End
      Begin Threed.SSCommand cmdAction 
         Height          =   360
         Index           =   4
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsmovimiento.frx":0060
      End
      Begin Threed.SSCommand cmdAction 
         Height          =   360
         Index           =   6
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsmovimiento.frx":007C
      End
      Begin Threed.SSCommand cmdAction 
         Height          =   360
         Index           =   7
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsmovimiento.frx":0098
      End
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsmovimiento.frx":00B4
      End
      Begin Threed.SSCommand cmdAction 
         Height          =   360
         Index           =   5
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsmovimiento.frx":00D0
      End
   End
   Begin Threed.SSPanel panToolBar 
      Align           =   1  'Align Top
      ...
      Begin Threed.SSCommand cmdConsulta 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsmovimiento.frx":00EC
      End
   ...
   End
End
Attribute VB_Name = "fPvsMovimiento"
...
Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  lblTitle.Caption = " Personal : " & Trim(o_PvsComxTieSer.dcaRegistro.Recordset!nombrepsn)
  If s_OptRegistro = "pvsvacacio" Then
    s_Registro = o_PvsVacaciones.dcaRegistro.Recordset!codpsn
    ' Cadenas de Texto, Recuperar Información
    s_Sql = "SELECT CONCAT(mov.pdocts, mov.subcts) AS cPrimaryKey, mov.codpsn, mov.pdocts, mov.subcts, sub.descrisub, mov.pdomes, "
    s_Sql = s_Sql & "mov.fechaini, mov.fechafin, mov.fechacan, mov.numeroanos, mov.numeromeses, mov.numerodias, mov.estadomov "
    s_Sql = s_Sql & "FROM plctsmovimiento mov "
    s_Sql = s_Sql & "INNER JOIN plctsperiodosub sub ON mov.codcls=sub.codcls AND mov.pdocts=sub.pdocts AND mov.subcts=sub.subcts "
    s_Sql = s_Sql & "WHERE mov.codcls='" & ps_ClsPlanilla & "' "
    s_Sql = s_Sql & "AND mov.codpsn='" & s_Registro & "' "
  End If
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  Select Case Index
   ...
   Case 6, 7  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = UCase$(s_TitleWindow)
    gdl_Procedure.ps_ReportName = "periodos"
    ...
    aElementos(0) = "{periodos.per_empresa}='" & ps_CodEmpresa & "' AND {periodos.per_ano}='" & ps_Ano & "'"
    aElementos(1) = ""
    
    gdl_Procedure.ParametersPrinter ps_StrgConnec & ps_DataBase, fMenu.CryReport, (Index - 6), False, False, False, True, False, aElemento, aElementos
  End Select

End Sub
Private Sub cmdConsulta_Click()
  ' Verifico que Existan Registros
  If dcaRegistro.Recordset.RecordCount = 0 Then Beep: MsgBox "No Existen " & s_TitleTable & " para Consultar", vbExclamation: Exit Sub
  fCtsConsultaCalculo.Show
End Sub
Private Sub dcaRegistro_MoveComplete(ByVal adReason As ADODB.EventReasonEnum, ByVal pError As ADODB.Error, adStatus As ADODB.EventStatusEnum, ByVal pRecordset As ADODB.Recordset)
  ...
  If FormVisible("fCtsConsultaCalculo") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      fCtsConsultaCalculo.RecuperaRegistros
    End If
  End If
  If FormVisible("fAbcCtsMovimiento") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      fAbcCtsMovimiento.ShowScreen
    End If
  End If
...
Private Sub Form_Load()
  ...
  Me.Height = 5640: Me.Width = 9020
  Me.Left = 800: Me.Top = 900
  ' Recupera parámetro
  gdl_Procedure.pl_RecordSelector = True
  
  ' Caso de instacia del formulario
  s_OptRegistro = Me.Tag
  
  ' Titulo del formulario y la Grilla
  s_TitleWindow = "Registro de Provisión de " & IIf(s_OptRegistro = "pvsvacacio", "Vacaciones", "Gratificaciones")
  s_TitleTable = "Movimiento de CTS"
  
  ReDim aElemento(9, 10)
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 0) = Choose(n_Index + 1, "Ejercicio", "Periodo", "Sub-Periodo", "Descripción", "Fec.Inicio", "Fec.Final", "Dias", "Fec.Cancelar", "Ok")
    aElemento(n_Index, 1) = Choose(n_Index + 1, "pdocts", "subcts", "", "descrisub", "fechaini", "fechafin", "numerodias", "fechacan", "estadomov")
    aElemento(n_Index, 2) = Choose(n_Index + 1, 750, 750, 750, 1500, 900, 900, 500, 900, 300)
    aElemento(n_Index, 3) = Choose(n_Index + 1, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbRightJustify, vbLeftJustify, vbCenter)
    aElemento(n_Index, 4) = Choose(n_Index + 1, "", "", "", "", s_FormatoFecha, s_FormatoFecha, "", s_FormatoFecha, "")
    aElemento(n_Index, 5) = Choose(n_Index + 1, dbgMergeRestricted, dbgMergeRestricted, False, False, False, False, False, False, False)
    aElemento(n_Index, 6) = Choose(n_Index + 1, True, True, True, True, True, True, True, True, True)
    aElemento(n_Index, 7) = Choose(n_Index + 1, "", "", "", "", "", "", "", "", "")
    aElemento(n_Index, 8) = Choose(n_Index + 1, dbgCenter, dbgTop, dbgTop, dbgTop, dbgTop, dbgTop, dbgTop, dbgTop, dbgTop)
  ...
  tdbRegistro.Columns(8).ValueItems.Presentation = dbgNormal
  tdbRegistro.Columns(8).ValueItems.Translate = True
  For n_Index = 0 To 2
    ' Estado del registro
    tdbRegistro.Columns(8).ValueItems.Add Item
    tdbRegistro.Columns(8).ValueItems.Item(n_Index).Value = Choose(n_Index + 1, s_Estado_Ina, s_Estado_Act, s_Estado_Blq)
    tdbRegistro.Columns(8).ValueItems.Item(n_Index).DisplayValue = LoadPicture(gdl_Procedure.ps_PathImagen & Choose(n_Index + 1, "periogen", "periopvs", "periocan") & ".bmp")
  Next n_Index
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fCtsConsultaCalculo") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fCtsConsultaCalculo.Caption, vbExclamation
    Cancel = True
  End If
  If FormVisible("fAbcCtsMovimiento") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcCtsMovimiento.Caption, vbExclamation
    Cancel = True
  End If
