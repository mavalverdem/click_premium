...
Begin VB.Form fRemunerExcepcional 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "remunexep.frx":00D0
      End
   End
End
Attribute VB_Name = "fRemunerExcepcional"
...
Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT rxe.codcpc, cpc.descpc, rxe.imporemune, rxe.codmon"
  s_Sql = s_Sql & " FROM plremuexce rxe, plconcepto cpc"
  s_Sql = s_Sql & " WHERE rxe.codcls='" & ps_ClsPlanilla & "'"
  s_Sql = s_Sql & " AND rxe.codpdo='" & Trim(o_SelExepcional.txtPeriodo) & "'"
  s_Sql = s_Sql & " AND rxe.codpsn='" & o_SelExepcional.dcaRegistro.Recordset!codpsn & "'"
  s_Sql = s_Sql & " AND cpc.codcpc=rxe.codcpc"
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  Select Case Index
   ...
   Case 6, 7  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = "MOVIMIENTOS EXEPCIONALES"
    gdl_Procedure.ps_ReportName = "rptremexepci"
    ReDim aElemento(3, 3): ReDim aElementos(2)
    ' Parametros del Reporte
    aElemento(0, 0) = ps_CodEmpresa
    aElemento(0, 1) = o_SelExepcional.tdbRegistro.Columns(0).DataField & " ASC"
    aElemento(0, 2) = ""
    ...
    aElemento(2, 2) = "Periodo;" & Trim(o_SelExepcional.txtPeriodo.Text) & " - " & Trim(o_SelExepcional.lblHelp(0).Caption) & ";true"
    ...
    s_Sql = s_Sql & " WHERE rex.codpdo='" & Trim(o_SelExepcional.txtPeriodo.Text) & "'"
    s_Sql = s_Sql & " AND psn.codcls='" & ps_ClsPlanilla & "'"
    s_Sql = s_Sql & " AND psn.codpsn='" & o_SelExepcional.tdbRegistro.Columns(0).Text & "'"
...
Private Sub Form_Load()
  ...
  o_SelExepcional.txtPeriodo.Locked = True
  o_SelExepcional.txtPeriodo.BackColor = &HC7D8E0
  o_SelExepcional.txtPeriodo.ForeColor = &HC00000
  o_SelExepcional.cmdHelp(0).Enabled = False
  ' Recupera parámetro
  gdl_Procedure.pl_RecordSelector = True
  
  ' Titulo del formulario y la Grilla
  s_TitleWindow = "Ingresos / Descuentos Exepcionales"
  s_TitleTable = "Remuneración Exepcional"
  
  ReDim aElemento(4, 10)
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 0) = Choose(n_Index + 1, "Código", "Descripción", "Mon", "Importe")
    aElemento(n_Index, 1) = Choose(n_Index + 1, "codcpc", "descpc", "codmon", "imporemune")
    aElemento(n_Index, 2) = Choose(n_Index + 1, 700, 2566.03, 450, 950)
    aElemento(n_Index, 3) = Choose(n_Index + 1, vbLeftJustify, vbLeftJustify, vbCenter, vbRightJustify)
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcRemunExepcion") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcRemunExepcion.Caption, vbExclamation
    Cancel = True
    Exit Sub
  End If
  ' Restauro los valores del periodo seleccionado
  o_SelExepcional.txtPeriodo.Locked = False
  o_SelExepcional.txtPeriodo.BackColor = &HFFFFFF
  o_SelExepcional.txtPeriodo.ForeColor = &HC00000
  o_SelExepcional.cmdHelp(0).Enabled = True
