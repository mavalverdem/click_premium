...
Begin VB.Form fAbcDistribuMoneda 
   BorderStyle     =   1  'Fixed Single
   ClientHeight    =   3645
   ClientLeft      =   2265
   ClientTop       =   375
   ClientWidth     =   7200
   Icon            =   "abcdimon.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin TabDlg.SSTab tabRegister 
      ...
      TabPicture(0)   =   "abcdimon.frx":000C
      ...
      Begin VB.ComboBox cmbMoneda 
         ...
         ForeColor       =   &H00000080&
         Height          =   315
         ItemData        =   "abcdimon.frx":0028
         Left            =   1340
         List            =   "abcdimon.frx":002A
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "abcdimon.frx":010C
      End
   End
End
Attribute VB_Name = "fAbcDistribuMoneda"
...
Private s_DistribuMoneda As String, s_Moneda As String  ' Codigo del registro
'[
...
If Me.Tag = s_MdoData_Ins Then
    gdl_Procedure.EditCombo "PK", cmbMoneda, -1, Me.Tag, False
    gdl_Procedure.EditText "PK", txtCodigo, FormatNumber(0, 2), Me.Tag, False, 18, vbRightJustify
    gdl_Procedure.EditText "AT", txtDescripcion, "", Me.Tag, False, fTablaSistema.dcaRegistro.Recordset!desdmo.DefinedSize
...
Else
    i = IIf(fTablaSistema.dcaRegistro.Recordset!codmon = "N", 0, 1)
    gdl_Procedure.EditCombo "PK", cmbMoneda, i, Me.Tag, True
    gdl_Procedure.EditText "PK", txtCodigo, FormatNumber(fTablaSistema.dcaRegistro.Recordset!valordmo, 2), Me.Tag, True, 18, vbRightJustify
    gdl_Procedure.EditText "AT", txtDescripcion, gdl_Funcion.aTexto(fTablaSistema.dcaRegistro.Recordset!desdmo), Me.Tag, False, fTablaSistema.dcaRegistro.Recordset!desdmo.DefinedSize
    gdl_Procedure.EditOptionCheck "AT", optEstado(0), (fTablaSistema.dcaRegistro.Recordset!estadodmo = s_Estado_Act), Me.Tag, True
    gdl_Procedure.EditOptionCheck "AT", optEstado(1), (fTablaSistema.dcaRegistro.Recordset!estadodmo = s_Estado_Ina), Me.Tag, True
End If
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  If MsgBox("¿ Estás Seguro de Eliminar el " & lblTitle & " '" & Trim$(txtDescripcion) & "' ?", vbCritical + vbYesNo + vbDefaultButton2) = vbYes Then
    ...
    s_DistribuMoneda = s_Moneda & Trim$(txtCodigo)
    
    '[ Inicio la conexión a la base de datos ]
    ps_StrgConnec = OpenConnection(ps_Servidor, ps_DataBase)
    ' Creo los arreglos de eliminacion
    a_Where = Array("codmon", "valordmo")
    a_Valores = Array(s_Moneda, FormatNumber(txtCodigo, 2))
    a_Tipos = Array(TipoDato.Caracter, TipoDato.Numero)
      
    gdl_Conexion.IniciaTransaccion    'Inicia transacción
    ' Elimino el registro
    If Not Records_Del("pldstmoneda", a_Where, a_Valores, a_Tipos) Then GoTo Error
    ...
    If Not l_ExistRecord Then
      fTablaSistema.dcaRegistro.Recordset.Find ("cPrimaryKey >= '" & s_DistribuMoneda & "'")
...
Private Sub cmdUpdate_Click()
  ...
  If CDec(txtCodigo) <= 0 Then Beep: MsgBox "Debe Ingresar el Valor " & lblTitle, vbExclamation: txtCodigo.SetFocus: Exit Sub
  ...
  a_Campos = Array("codmon", "valordmo", "desdmo", "estadodmo", IIf(Me.Tag = s_MdoData_Ins, "usrcre", "usrmdf"), IIf(Me.Tag = s_MdoData_Ins, "fyhcre", "fyhmdf"))
  a_Valores = Array(s_Moneda, CDec(txtCodigo), Trim$(txtDescripcion), s_Estado, ps_Usuario, Format(Now, s_FmtFeHoMysql_0))
  ...
  If Me.Tag = s_MdoData_Ins Then
    If Not Records_Ins("pldstmoneda", a_Campos, a_Valores, a_Tipos) Then GoTo Error
  Else
    If Not Records_Upd("pldstmoneda", a_Campos, a_Valores, a_Tipos, a_Where) Then GoTo Error
  End If
  gdl_Conexion.ConfirmaTransaccion ' Confirma transacción
...
Private Sub Form_Load()
  ...
  Me.Height = 4120: Me.Width = 7290
  Me.Left = 1080: Me.Top = 1500
  
  ' Titulo del formulario y panel
  s_TitleWindow = "Actualización Distribución Monetaria"
  lblTitle = "Distribución Monetaria"
  ' Coloco el puntero en espera
  gdl_Procedure.PunteroEnEspera
  ...
  If Not l_ExistRecord Then s_ParCodigo = fTablaSistema.dcaRegistro.Recordset!valordmo
  
  ' Adiciono las monedas
  For i = 0 To 1
    cmbMoneda.AddItem Choose(i + 1, s_Codmon_mn_Nom, s_Codmon_me_Nom)
  Next i
...
Private Sub txtCodigo_Validate(Cancel As Boolean)
  txtCodigo.Text = IIf(Not IsNumeric(txtCodigo.Text), 0, txtCodigo.Text)
  txtCodigo.Text = FormatNumber(CDec(txtCodigo.Text), 2)
End Sub
