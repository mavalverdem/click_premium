...
Begin VB.Form fProcesoCalculo 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "tipocalculo.frx":0108
      End
   End
End
Attribute VB_Name = "fProcesoCalculo"
...
Private Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT codproce, desproce, estadoproce "
...
End Sub
Private Sub ActivaProceso(ByVal s_ProcesoCalculo As String)
  ...
  ' Los procesos en inactivos
  s_Sql = "UPDATE plproceso SET estadoproce='" & s_Estado_Ina & "'"
  s_Sql = s_Sql & " WHERE codcls='" & ps_ClsPlanilla & "'"
  s_Sql = s_Sql & " AND estadoproce<>'" & s_EstadoRemAper & "'"
  If Not gdl_Conexion.Execucion(s_Sql, Modifica) Then GoTo Error
  ' Proceso slecciono activo
  s_Sql = "UPDATE plproceso SET estadoproce='" & s_Estado_Act & "'"
  s_Sql = s_Sql & " WHERE codcls='" & ps_ClsPlanilla & "'"
  s_Sql = s_Sql & " AND codproce='" & s_ProcesoCalculo & "'"
  If Not gdl_Conexion.Execucion(s_Sql, Modifica) Then GoTo Error
  gdl_Conexion.ConfirmaTransaccion ' Confirma transacción
  ' Refresco el ado control y la grilla
  gdl_Procedure.RefreshAdoControl dcaRegistro, tdbRegistro, tdbRegistro.Caption
  ' Ubico el registro actualizado
  dcaRegistro.Recordset.Find ("codproce='" & s_ProcesoCalculo & "'")
  GoTo Finalizar
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  Select Case Index
   ...
   Case 6, 7  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = "PROCESOS DE CALCULO"
    gdl_Procedure.ps_ReportName = "lstprocecalculo"
    ...
    ' ]
   Case 8 ' Asignación de concepto de planilla
    If dcaRegistro.Recordset.RecordCount = 0 Then Beep: MsgBox "No Existen " & s_TitleTable$ & " para Asisgnar", vbExclamation: Exit Sub
    If dcaRegistro.Recordset("estadoproce") = s_EstadoRemAper Then Beep: MsgBox "Proceso " & s_TitleTable$ & " Bloquedo", vbExclamation: Exit Sub
    Me.Tag = s_MdoData_Vis
    fAbcConcepProceso.Show
   Case 9 ' Seleccion de proceso de cálculo
    If dcaRegistro.Recordset.RecordCount = 0 Then Beep: MsgBox "No Existen " & s_TitleTable$ & " para Asisgnar", vbExclamation: Exit Sub
    If dcaRegistro.Recordset("estadoproce") = s_EstadoRemAper Then Beep: MsgBox "Proceso " & s_TitleTable$ & " Bloquedo", vbExclamation: Exit Sub
    ActivaProceso dcaRegistro.Recordset!codproce
  End Select
...
Private Sub dcaRegistro_MoveComplete(ByVal adReason As ADODB.EventReasonEnum, ByVal pError As ADODB.Error, adStatus As ADODB.EventStatusEnum, ByVal pRecordset As ADODB.Recordset)
  ...
  If FormVisible("fAbcProcesoCalculo") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      fAbcProcesoCalculo.ShowScreen
    End If
  End If
  If FormVisible("fAbcConcepProceso") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      fAbcConcepProceso.ShowScreen
    End If
  End If
...
Private Sub Form_Load()
  ...
  For i = 0 To 2
    ...
    tdbRegistro.Columns(2).ValueItems.Item(i).Value = Choose(i + 1, s_Estado_Act, s_Estado_Ina, s_EstadoRemAper)
    tdbRegistro.Columns(2).ValueItems.Item(i).DisplayValue = LoadPicture(gdl_Procedure.ps_PathImagen & Choose(i + 1, "estadok", "estadnok", "perionok") & ".bmp")
  Next i
  ...
  For i = 0 To (UBound(aElemento, 1) - 1)
    aElemento(i, 1) = Choose(i + 1, "seleccio", "anadir", "borrar", "ordascen", "orddesce", "busqueda", "prelimin", "Imprimir", "analsald", "activar")
    aElemento(i, 2) = Choose(i + 1, "Selecciona y Edita " & s_TitleTable, "Añadir " & s_TitleTable, "Eliminar " & s_TitleTable, "Ordenar Ascendente", "Ordenar Descendente", "Buscar " & s_TitleTable$, "Presentación Preliminar", "Imprimir", "Asigna Conceptos de Planilla", "Activar " & s_TitleTable)
  Next i
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcProcesoCalculo") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcProcesoCalculo.Caption, vbExclamation
    Cancel = True
  End If
