...
Begin VB.Form fPvsVacacion 
   ...
   End
End
Attribute VB_Name = "fPvsVacacion"
...
Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  lblTitle.Caption = " Personal : " & Trim(o_PvsVacaciones.dcaRegistro.Recordset!nombrepsn)
  s_Registro = o_PvsVacaciones.dcaRegistro.Recordset!codpsn
  ' Cadenas de Texto, Recuperar Información
  s_Sql = "SELECT CONCAT(vac.codpvs, vac.pdopvs) AS cPrimaryKey, vac.codpsn, vac.codpvs, vac.pdopvs, "
  s_Sql = s_Sql & "pvs.descripvs, vac.fechaini, vac.fechafin, vac.numerodias, vac.estadovac "
  s_Sql = s_Sql & "FROM plpvsvacacion vac "
  s_Sql = s_Sql & "INNER JOIN plpvsperiodovac pvs ON vac.codcls=pvs.codcls AND vac.codpvs=pvs.codpvs "
  s_Sql = s_Sql & "WHERE vac.codcls='" & ps_ClsPlanilla & "' "
  s_Sql = s_Sql & "AND vac.codpsn='" & s_Registro & "' "
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  Select Case Index
   ...
   Case 6, 7  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = UCase$("Provisión de Vacaciones")
    gdl_Procedure.ps_ReportName = "cstpvsvacacion"
    ...
    aElemento(2, 2) = "Periodo;" & "Provisión - " & ps_Anyo & ";true"
    ...
    s_Sql = s_Sql & "AND vac.codpsn='" & o_PvsVacaciones.dcaRegistro.Recordset!codpsn & "' "
    s_Sql = s_Sql & "ORDER BY vac.pdoano, vac." & aElemento(0, 1)
...
Private Sub cmdConsulta_Click()
  ...
  fMenu.Tag = "pvsvacacio"
  o_PvsVacaConsul.Show
End Sub
...
Private Sub Form_Load()
  ...
  s_TitleWindow = "Registro de Provisión de Vacaciones"
  s_TitleTable = "Provisión de Vacaciones"
  
  ReDim aElemento(7, 10)
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 0) = Choose(n_Index + 1, "Periodo", "Descripción", "Sub-Periodo", "Fec.Inicio", "Fec.Final", "Dias", "Ok")
    aElemento(n_Index, 1) = Choose(n_Index + 1, "codpvs", "descripvs", "pdopvs", "fechaini", "fechafin", "numerodias", "estadovac")
    aElemento(n_Index, 2) = Choose(n_Index + 1, 750, 2000, 950, 950, 950, 540, 300)
    aElemento(n_Index, 3) = Choose(n_Index + 1, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbRightJustify, vbCenter)
  ...
  For n_Index = 0 To 2
    ...
    tdbRegistro.Columns(6).ValueItems.Item(n_Index).Value = Choose(n_Index + 1, s_Estado_Ina, s_Estado_Act, s_Estado_Blq)
    tdbRegistro.Columns(6).ValueItems.Item(n_Index).DisplayValue = LoadPicture(gdl_Procedure.ps_PathImagen & Choose(n_Index + 1, "periogen", "periopvs", "periocan") & ".bmp")
  Next n_Index
  ...
  RecuperaRegistros "codpvs, pdopvs ASC"
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcPvsVacacion") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcPvsVacacion.Caption, vbExclamation
    Cancel = True
  End If
  If FormVisible("fConsultaCalculo") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & o_PvsVacaConsul.Caption, vbExclamation
    Cancel = True
  End If
