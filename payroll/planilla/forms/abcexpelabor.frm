...
Begin VB.Form fAbcExperienciaLaboral 
   BorderStyle     =   1  'Fixed Single
   ClientHeight    =   4020
   ClientLeft      =   2265
   ClientTop       =   375
   ClientWidth     =   7200
   Icon            =   "abcexpelabor.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin TabDlg.SSTab tabRegister 
      ...
      TabPicture(0)   =   "abcexpelabor.frx":000C
      ...
      Tab(0).Control(6)=   "dtpFechas(1)"
      Tab(0).Control(6).Enabled=   0   'False
      Tab(0).Control(7)=   "dtpFechas(0)"
      Tab(0).Control(7).Enabled=   0   'False
      Tab(0).Control(8)=   "cmdHelp(0)"
      Tab(0).Control(8).Enabled=   0   'False
      Tab(0).Control(9)=   "txtCargo"
      Tab(0).Control(9).Enabled=   0   'False
      Tab(0).Control(10)=   "txtEmpresa"
   ...
   End
End
Attribute VB_Name = "fAbcExperienciaLaboral"
...
Sub ShowScreen()
  ...
  If Me.Tag = s_MdoData_Ins Then
    gdl_Procedure.EditText "AT", txtEmpresa, "", Me.Tag, False, 50
    gdl_Procedure.EditText "AT", txtCargo, "", Me.Tag, False, 2
  ...
  Else
    gdl_Procedure.EditText "AT", txtEmpresa, fAbcPersonal.tdbExperiencia.Columns(0).Text, Me.Tag, False, 50
    gdl_Procedure.EditText "AT", txtCargo, fAbcPersonal.tdbExperiencia.Columns(4).Text, Me.Tag, False, 2
    gdl_Procedure.EditDTPicker "AT", dtpFechas(0), fAbcPersonal.tdbExperiencia.Columns(2).Text, Me.Tag, False, s_FormatoFecha, dtpShortDate
    gdl_Procedure.EditDTPicker "AT", dtpFechas(1), fAbcPersonal.tdbExperiencia.Columns(3).Text, Me.Tag, False, s_FormatoFecha, dtpShortDate
    gdl_Procedure.EditText "AT", txtObservacion, gdl_Funcion.aTexto(fAbcPersonal.tdbExperiencia.Columns(5).Text), Me.Tag, False, 100
  End If
  lblHelp(0) = gdl_Funcion.DameDescripcion(ps_StrgConnec & ps_DataBase, ps_ClsPlanilla, txtCargo, "DC")
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  If MsgBox("¿ Estás Seguro de Eliminar el " & lblTitle & "' ?", vbCritical + vbYesNo + vbDefaultButton2) = vbYes Then
    ...
    n_Secuencia = CInt(fAbcPersonal.tdbExperiencia.Columns(6).Text)
    ...
    ' Elimino el registro
    If Not Records_Del("plexpelaboral", a_Where, a_Valores, a_Tipos) Then GoTo Error
    gdl_Conexion.ConfirmaTransaccion  'Confirma transacción
    
    MsgBox "Se Elimino exitosamente " & lblTitle, vbInformation
    ' Refresco el ado control y la grilla
    fAbcPersonal.RecuperarExperiencia
    ' Verifico si aun existen registros
    l_ExistRecord = ((fAbcPersonal.tdbExperiencia.EOF And fAbcPersonal.tdbExperiencia.BOF) Or fAbcPersonal.tdbExperiencia.VisibleRows = 0)
    If Not l_ExistRecord Then
'      fRemunerExcepcional.dcaRegistro.Recordset.Find ("codcpc >= '" & s_ConceptoPlanilla & "'")
      If fAbcPersonal.tdbExperiencia.EOF Then fAbcPersonal.tdbExperiencia.MoveLast
    Else
      Unload Me
    End If
  End If
...
Private Sub cmdMove_Click(Index As Integer)
  ...
  Select Case Index
   Case 0: fAbcPersonal.tdbExperiencia.MoveFirst
   Case 1: If Not fAbcPersonal.tdbExperiencia.BOF Then fAbcPersonal.tdbExperiencia.MovePrevious
           If fAbcPersonal.tdbExperiencia.BOF Then fAbcPersonal.tdbExperiencia.MoveFirst
   Case 2: If Not fAbcPersonal.tdbExperiencia.EOF Then fAbcPersonal.tdbExperiencia.MoveNext
           If fAbcPersonal.tdbExperiencia.EOF Then fAbcPersonal.tdbExperiencia.MoveLast
   Case 3: fAbcPersonal.tdbExperiencia.MoveLast
  End Select
...
Private Sub cmdUpdate_Click()
  ...
  If txtEmpresa = "" Then Beep: MsgBox "Debe Ingresar la Empresa " & lblTitle, vbExclamation: txtEmpresa.SetFocus: Exit Sub
  If txtCargo = "" Then Beep: MsgBox "Debe Ingresar el Cargo " & lblTitle, vbExclamation: txtCargo.SetFocus: Exit Sub
  If lblHelp(0) = "???" Then Beep: MsgBox "Cargo de Personal no es valido; Verificar", vbExclamation: txtCargo.SetFocus: Exit Sub
  ...
  n_Secuencia = CInt(Val(fAbcPersonal.tdbExperiencia.Columns(6).Text))
  If Me.Tag = s_MdoData_Ins Then
    s_Sql = "SELECT IFNULL(MAX(orden), 0)+1 AS registro "
    s_Sql = s_Sql & "FROM plexpelaboral "
  ...
  a_Campos = Array("codcls", "codpsn", "orden", "empresa", "codcgo", "fechaini", "fechafin", "observacion", IIf(Me.Tag = s_MdoData_Ins, "usrcre", "usrmdf"), IIf(Me.Tag = s_MdoData_Ins, "fyhcre", "fyhmdf"))
  a_Valores = Array(ps_ClsPlanilla, s_Registro, n_Secuencia, Trim(txtEmpresa.Text), Trim(txtCargo.Text), Format(dtpFechas(0), s_FmtFechMysql_0), Format(dtpFechas(1), s_FmtFechMysql_0), Trim(txtObservacion.Text), ps_Usuario, Format(Now, s_FmtFeHoMysql_0))
  ...
  If Me.Tag = s_MdoData_Ins Then
    If Not Records_Ins("plexpelaboral", a_Campos, a_Valores, a_Tipos) Then GoTo Error
  Else
    If Not Records_Upd("plexpelaboral", a_Campos, a_Valores, a_Tipos, a_Where) Then GoTo Error
  End If
  gdl_Conexion.ConfirmaTransaccion ' Confirma transacción
...
Private Sub Form_Load()
  ...
  s_TitleWindow = "Actualización Experiencia Laboral"
  lblTitle = "Experiencia Laboral"
  n_IndexHelp = -1
  ' Coloco el puntero en espera
  gdl_Procedure.PunteroEnEspera

  ' Obtengo el modo de operación del registro
  Me.Tag = fAbcPersonal.tdbExperiencia.Tag
  ...
  l_ExistRecord = (fAbcPersonal.tdbExperiencia.EOF Or fAbcPersonal.tdbExperiencia.BOF)
  If Not l_ExistRecord Then s_ParCodigo = fAbcPersonal.tdbExperiencia.Columns(0).Text
...
Private Sub tdbHelp_DblClick()
  ...
  If porstHelp.RecordCount = 0 Or (porstHelp.EOF And porstHelp.BOF) Then
    ...
    Exit Sub
  End If
  txtCargo.Text = tdbHelp.Columns(0).Value
...
Private Sub txtEmpresa_KeyPress(KeyAscii As Integer)
  ...
  If KeyAscii = vbKeyReturn Then
    If txtEmpresa = "" Then
      Beep
      MsgBox "Debe Ingresar la empresa de " & lblTitle, vbExclamation
      txtEmpresa.SetFocus
    Else
      txtCargo.SetFocus
      KeyAscii = 0
