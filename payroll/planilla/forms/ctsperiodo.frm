...
Private Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT pdocts, descricts, pdoano, estadocts "
  s_Sql = s_Sql & "FROM plctsperiodo "
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  Select Case Index
   ...
   Case 6, 7  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = "ENTIDAD DE FONDO DE PENSION"
    gdl_Procedure.ps_ReportName = "lstentidadafp"
    ...
    s_Sql = s_Sql & " SELECT afp.codafp, afp.desafp, afp.factor1, afp.factor2,"
    s_Sql = s_Sql & " afp.factor3 , afp.factor4, afp.codbco, bco.desbco, afp.ctacteafp,"
    s_Sql = s_Sql & " afp.desctacteafp, afp.ctactefondo, afp.desctactefondo, afp.estadoafp"
    s_Sql = s_Sql & " FROM plentidadafp afp"
    s_Sql = s_Sql & " LEFT JOIN plbanco bco ON afp.codbco=bco.codbco"
    s_Sql = s_Sql & " ORDER BY " & aElemento(0, 1)
    gdl_Funcion.Execution ps_StrgConnec & ps_DataBase, s_Sql
    
    ' Ejecuto el reporto y elimino la tebla temporal
    ...
    gdl_Funcion.Execution ps_StrgConnec & ps_DataBase, s_Sql
    ' ]
   Case 8     ' Sub periodos de provisión
    ' Verifico que Existan Registros
    If dcaRegistro.Recordset.RecordCount = 0 Then Beep: MsgBox "No Existen " & s_TitleTable & " para Generar", vbExclamation: Exit Sub
    fCtsPeriodoSub.Show
  End Select
...
Private Sub dcaRegistro_MoveComplete(ByVal adReason As ADODB.EventReasonEnum, ByVal pError As ADODB.Error, adStatus As ADODB.EventStatusEnum, ByVal pRecordset As ADODB.Recordset)
  ...
  If FormVisible("fAbcCtsPeriodo") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      fAbcCtsPeriodo.ShowScreen
    End If
  End If
  If FormVisible("fCtsPeriodoSub") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      fCtsPeriodoSub.RecuperaRegistros "subcts"
    End If
  End If
...
Private Sub Form_Load()
  ...
  Me.Left = 105: Me.Top = 680
  ' Recupera parámetro
  gdl_Procedure.pl_RecordSelector = True
  
  ' Titulo del formulario y la Grilla
  s_TitleWindow = "Registro Periodo de Provisión"
  s_TitleTable = "Periodo Provisión"
  
  ReDim aElemento(3, 10)
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 0) = Choose(n_Index + 1, "Código", "Descripción", "Ok")
    aElemento(n_Index, 1) = Choose(n_Index + 1, "pdocts", "descricts", "estadocts")
    aElemento(n_Index, 2) = Choose(n_Index + 1, 800, 3566.03, 300)
  ...
  For n_Index = 0 To 2
    ...
    tdbRegistro.Columns(2).ValueItems.Item(n_Index).Value = Choose(n_Index + 1, s_Estado_Ina, s_Estado_Act, s_Estado_Blq)
    tdbRegistro.Columns(2).ValueItems.Item(n_Index).DisplayValue = LoadPicture(gdl_Procedure.ps_PathImagen & Choose(n_Index + 1, "periogen", "periopvs", "periocan") & ".bmp")
  Next n_Index
  ...
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
      aElemento(n_Index, 1) = Choose(n_Index + 1, "seleccio", "anadir", "borrar", "ordascen", "orddesce", "busqueda", "prelimin", "Imprimir", "dividir")
      aElemento(n_Index, 2) = Choose(n_Index + 1, "Selecciona y Edita " & s_TitleTable, "Añadir " & s_TitleTable, "Eliminar " & s_TitleTable, "Ordenar Ascendente", "Ordenar Descendente", "Buscar " & s_TitleTable$, "Presentación Preliminar", "Imprimir", "Sub Periodos Provisión")
  Next n_Index
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcCtsPeriodo") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcCtsPeriodo.Caption, vbExclamation
    Cancel = True
  End If
  If FormVisible("fCtsPeriodoSub") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fCtsPeriodoSub.Caption, vbExclamation
    Cancel = True
  End If
