...
Begin VB.Form fEmpresa 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "empresa.frx":00EC
      End
   End
End
Attribute VB_Name = "fEmpresa"
...
Private Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT DISTINCTROW emp.codemp, emp.razemp, emp.rucemp, emp.siscon, emp.sispla, emp.estemp, emp.actividad, emp.empcon, emp.nombredbemp "
  s_Sql = s_Sql & "FROM tgemp emp, sgpms exu "
  s_Sql = s_Sql & "WHERE exu.codsis='" & ps_CodSistema & "' "
  s_Sql = s_Sql & "AND exu.codemp=emp.codemp "
  s_Sql = s_Sql & "AND exu.codusr='" & ps_Usuario & "' "
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  Select Case Index
   ...
   Case 6, 7  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = "EMPRESAS DE SISTEMA"
    gdl_Procedure.ps_ReportName = "lstempresa"
    ...
    gdl_Funcion.Execution ps_StrgConnec & ps_DataBase, s_Sql
    ' ]
    Case 8    ' Parametros de configuración
      ' Verifico que Existan Registros
      If dcaRegistro.Recordset.RecordCount = 0 Then Beep: MsgBox "No Existen " & s_TitleTable & " para Configurar", vbExclamation: Exit Sub
      fPrmEmpresa.Show vbModal
  End Select
...
Private Sub Form_Load()
  ...
  s_TitleTable = "Empresas del Sistema"
  
  ReDim aElemento(5, 10)
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 0) = Choose(n_Index + 1, "Código", "Razón Social", "RUC", "Ok", "Actividad")
    aElemento(n_Index, 1) = Choose(n_Index + 1, "codemp", "razemp", "rucemp", "sispla", "actividad")
    aElemento(n_Index, 2) = Choose(n_Index + 1, 705.2599, 3585.26, 1170.142, 300, 0)
    aElemento(n_Index, 3) = Choose(n_Index + 1, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbCenter, vbCenter)
    ...
    aElemento(n_Index, 9) = Choose(n_Index + 1, 0, 0, 0, 0, 1)
  Next n_Index
  ...
  For n_Index = 0 To 1
    ...
    tdbRegistro.Columns(3).ValueItems.Item(n_Index).Value = Choose(n_Index + 1, s_Estado_Act, s_Estado_Ina)
  ...
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
      aElemento(n_Index, 1) = Choose(n_Index + 1, "seleccio", "anadir", "borrar", "ordascen", "orddesce", "busqueda", "prelimin", "Imprimir", "promedio")
      aElemento(n_Index, 2) = Choose(n_Index + 1, "Selecciona y Edita " & s_TitleTable, "Añadir " & s_TitleTable, "Eliminar " & s_TitleTable, "Ordenar Ascendente", "Ordenar Descendente", "Buscar " & s_TitleTable$, "Presentación Preliminar", "Imprimir", "Configuración Parametros")
  Next n_Index
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcEmpresa") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcEmpresa.Caption, vbExclamation
    Cancel = True
  End If
  If FormVisible("fPrmEmpresa") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fPrmEmpresa.Caption, vbExclamation
    Cancel = True
  End If
