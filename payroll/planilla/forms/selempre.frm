...
Begin VB.Form fSelEmpresa 
   BorderStyle     =   1  'Fixed Single
   ClientHeight    =   4275
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6735
   Icon            =   "selempre.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
...
Private Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT DISTINCTROW emp.codemp, emp.razemp, emp.rucemp, emp.siscon, emp.empcon, emp.nombredbemp "
  s_Sql = s_Sql & "FROM tgemp emp, sgpms oxu "
  s_Sql = s_Sql & "WHERE oxu.codsis='" & ps_CodSistema & "' "
  s_Sql = s_Sql & "AND oxu.codemp=emp.codemp "
  s_Sql = s_Sql & "AND oxu.codusr='" & ps_Usuario & "' "
  s_Sql = s_Sql & "AND emp.sispla='" & s_Estado_Act & "' "
  s_Sql = s_Sql & "ORDER BY " & s_Orden
  gdl_Procedure.SeteaAdoControl ps_StrgConnec & ps_BDSystems, dcaSeleccion, tdbSeleccion, s_Sql, adCmdText, adLockReadOnly
...
Select Case Index
  Case 0 ' Selecciona registro
    ...
    ps_CodEmpresa = gdl_Funcion.aTexto(dcaSeleccion.Recordset!codemp)
    ps_NomEmpresa = UCase$(gdl_Funcion.aTexto(dcaSeleccion.Recordset!razemp))
    ps_RucEmpresa = gdl_Funcion.aTexto(dcaSeleccion.Recordset!rucemp)
    ps_DataBase = gdl_Funcion.aTexto(dcaSeleccion.Recordset!nombredbemp)
    ps_DaBasCon = IIf(dcaSeleccion.Recordset!siscon = s_Estado_Act, "c" & ps_CodEmpresa & ps_Anyo, ps_DataBase)
    ps_EmpresaCon = gdl_Funcion.aTexto(dcaSeleccion.Recordset!empcon)
    Unload Me
    ' Visualizo las clases de planillas
...
Private Sub Form_Load()
  ...
  s_TitleWindow = "Selección de Empresas"
  s_TitleTable = "Empresas"
  ReDim aElemento(3, 10)
  For i = 0 To (UBound(aElemento, 1) - 1)
    aElemento(i, 0) = Choose(i + 1, "Código", "Razón Social", "RUC")
    aElemento(i, 1) = Choose(i + 1, "codemp", "razemp", "rucemp")
    aElemento(i, 2) = Choose(i + 1, 705.2599, 3585.26, 1170.142)
    aElemento(i, 3) = Choose(i + 1, vbLeftJustify, vbLeftJustify, vbLeftJustify)
