...
Begin VB.Form fPeriodoPago 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "peripago.frx":00EC
      End
   End
End
Attribute VB_Name = "fPeriodoPago"
...
Private Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT codpdo, despdo, fechaini, fechafin, fechapago, "
...
Private Sub cmdAction_Click(Index As Integer)
  Dim s_Registro As String, s_Estado As String * 1
  ...
  Select Case Index
   ...
   Case 6, 7  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = "PERIODOS DE PAGO"
    gdl_Procedure.ps_ReportName = "lstperiodopago"
    ...
    ' ]
   Case 8 ' Bloquear/desbloquear registro
    If dcaRegistro.Recordset.RecordCount = 0 Then Beep: MsgBox "No Existen " & s_TitleTable & " para actualizar", vbExclamation: Exit Sub
    If gdl_Funcion.aTexto(dcaRegistro.Recordset!estadopdo) = s_Estado_Ina Then Beep: MsgBox s_TitleTable & " no se puede actualizar", vbExclamation: Exit Sub
    n_Index = gdl_Funcion.aTexto(dcaRegistro.Recordset!estadopdo)
    s_Sql = "SELECT COUNT(*) AS registros FROM plperiodo "
    s_Sql = s_Sql & "WHERE codcls='" & ps_ClsPlanilla & "' "
    s_Sql = s_Sql & "AND anopdo='" & ps_Anyo & "' "
    s_Sql = s_Sql & "AND codpdo" & Choose(n_Index, "<", ">") & "'" & dcaRegistro.Recordset!codpdo & "' "
    s_Sql = s_Sql & "AND estadopdo" & Choose(n_Index, "<>", "=") & "'" & s_Estado_Blq & "'"
    Set porstRecordset = OpenRecordset(ps_StrgConnec & ps_DataBase, adOpenForwardOnly, adLockReadOnly, adUseClient, s_Sql)
    If porstRecordset!registros > 0 Then Beep: MsgBox "Actualizar " & s_TitleTable & " continuo", vbCritical: Exit Sub
    Beep
    If MsgBox("¿ Estás Seguro de " & Choose(n_Index, "Bloquear", "Desbloquear") & " el Periodo de Pago '" & Trim$(dcaRegistro.Recordset!despdo) & "' ?", vbCritical + vbYesNo + vbDefaultButton2) = vbYes Then
      ' Coloco el puntero en espera
      gdl_Procedure.PunteroEnEspera
      ' Capturo el registro a eliminar
      s_Registro = Trim(dcaRegistro.Recordset!codpdo)
      s_Estado = Choose(n_Index, s_Estado_Blq, s_Estado_Act)
      
      '[ Inicio la conexión a la base de datos ]
      ps_StrgConnec = OpenConnection(ps_Servidor, ps_DataBase)
      ' Creo los arreglos de eliminacion
      a_Campos = Array("codcls", "codpdo", "estadopdo")
      a_Valores = Array(ps_ClsPlanilla, s_Registro, s_Estado)
      ...
      dcaRegistro.Recordset.Find ("codpdo >= '" & s_Registro & "'")
    End If
    GoTo Finalizar
Error:
...
Private Sub Form_Load()
  ...
  For n_Index = 0 To 2
    ...
    tdbRegistro.Columns(4).ValueItems.Item(n_Index).Value = Choose(n_Index + 1, s_Estado_Ina, s_Estado_Act, s_Estado_Blq)
    tdbRegistro.Columns(4).ValueItems.Item(n_Index).DisplayValue = LoadPicture(gdl_Procedure.ps_PathImagen & Choose(n_Index + 1, "procenok", "proceok", "perioblk") & ".bmp")
  Next n_Index
  ...
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 1) = Choose(n_Index + 1, "seleccio", "anadir", "borrar", "ordascen", "orddesce", "busqueda", "prelimin", "Imprimir", "segurida")
    aElemento(n_Index, 2) = Choose(n_Index + 1, "Selecciona y Edita " & s_TitleTable, "Añadir " & s_TitleTable, "Eliminar " & s_TitleTable, "Ordenar Ascendente", "Ordenar Descendente", "Buscar " & s_TitleTable$, "Presentación Preliminar", "Imprimir", "Bloquear")
    aElemento(n_Index, 3) = Choose(n_Index + 1, "&s", "&n", "&e", "&a", "&d", "&b", "&v", "&i", "&b")
  Next n_Index
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcPeriodoPago") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcPeriodoPago.Caption, vbExclamation
    Cancel = True
  End If
