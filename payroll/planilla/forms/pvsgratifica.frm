...
Begin VB.Form fPvsGratificacion 
   ...
   End
End
Attribute VB_Name = "fPvsGratificacion"
...
Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  lblTitle.Caption = " Personal : " & Trim(o_PvsGratifica.dcaRegistro.Recordset!nombrepsn)
  s_Registro = o_PvsGratifica.dcaRegistro.Recordset!codpsn
  ' Cadenas de Texto, Recuperar Información
  s_Sql = "SELECT DISTINCTROW gra.pdoano, gra.sempvs, gra.codpsn, "
  s_Sql = s_Sql & "pdo.descripvs, pdo.mesini, pdo.mesfin, pdo.estadopvs "
  s_Sql = s_Sql & "FROM plpvsperiodogra pdo "
  s_Sql = s_Sql & "INNER JOIN plpvsgratifica gra ON pdo.codcls=gra.codcls AND pdo.pdoano=gra.pdoano AND pdo.sempvs=gra.sempvs "
  s_Sql = s_Sql & "WHERE pdo.codcls='" & ps_ClsPlanilla & "' "
  s_Sql = s_Sql & "AND pdo.pdoano='" & ps_Anyo & "' "
  s_Sql = s_Sql & "AND gra.codpsn='" & s_Registro & "' "
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  Select Case Index
   Case 0     ' Visualizar o analizar  registro
    ...
    fMenu.Tag = "pvsgratifi"
    o_PvsGratiConsul.Show
   ...
   Case 4, 5  ' Opciones de impresión
    ...
    gdl_Procedure.ps_ReportTitle = UCase$("Provisión de Gratificaciones")
    gdl_Procedure.ps_ReportName = "cstpvsgratifi"
    ...
    aElemento(2, 2) = "Periodo;" & "Ejercicio - " & ps_Anyo & ";true"
    ...
    s_Sql = s_Sql & "AND gra.codpsn='" & o_PvsGratifica.dcaRegistro.Recordset!codpsn & "' "
    s_Sql = s_Sql & "ORDER BY gra.sempvs, gra.pdomes"
...
Private Sub dcaRegistro_MoveComplete(ByVal adReason As ADODB.EventReasonEnum, ByVal pError As ADODB.Error, adStatus As ADODB.EventStatusEnum, ByVal pRecordset As ADODB.Recordset)
  ...
  If FormVisible("fConsultaCalculo") Then
    If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
      o_PvsGratiConsul.RecuperaRegistros
    End If
  End If
...
Private Sub Form_Load()
  ...
  Me.Height = 5640: Me.Width = 7970
  Me.Left = 1500: Me.Top = 900
  ' Recupera parámetro
  gdl_Procedure.pl_RecordSelector = True
  
  ' Titulo del formulario y la Grilla
  s_TitleWindow = "Registro de Provisión de Gratificación"
  s_TitleTable = "Provisión de Gratificación"
  
  ReDim aElemento(6, 10)
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 0) = Choose(n_Index + 1, "Periodo", "Semestre", "Descripción", "Mes Inicio", "Mes Final", "Ok")
    aElemento(n_Index, 1) = Choose(n_Index + 1, "pdoano", "sempvs", "descripvs", "mesini", "mesfin", "estadopvs")
    aElemento(n_Index, 2) = Choose(n_Index + 1, 950, 850, 2430, 950, 950, 300)
    aElemento(n_Index, 3) = Choose(n_Index + 1, vbCenter, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbCenter)
    aElemento(n_Index, 4) = Choose(n_Index + 1, "", "", "", "", "", "")
    aElemento(n_Index, 5) = Choose(n_Index + 1, dbgMergeRestricted, False, False, False, False, False)
    aElemento(n_Index, 6) = Choose(n_Index + 1, True, True, True, True, True, True)
    aElemento(n_Index, 7) = Choose(n_Index + 1, "", "", "", "", "", "")
    aElemento(n_Index, 8) = Choose(n_Index + 1, dbgCenter, dbgTop, dbgTop, dbgTop, dbgTop, dbgTop)
  ...
  For n_Index = 0 To 11
    ...
    tdbRegistro.Columns(3).ValueItems.Item(n_Index).Value = Format(n_Index + 1, "00")
    tdbRegistro.Columns(3).ValueItems.Item(n_Index).DisplayValue = Choose(n_Index + 1, "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Setiembre", "Octubre", "Noviembre", "Diciembre")
    ' Mes Final
    tdbRegistro.Columns(4).ValueItems.Add Item
    tdbRegistro.Columns(4).ValueItems.Item(n_Index).Value = Format(n_Index + 1, "00")
    tdbRegistro.Columns(4).ValueItems.Item(n_Index).DisplayValue = Choose(n_Index + 1, "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Setiembre", "Octubre", "Noviembre", "Diciembre")
  Next n_Index
  For n_Index = 0 To 2
    ' Estado del registro
    tdbRegistro.Columns(5).ValueItems.Add Item
    tdbRegistro.Columns(5).ValueItems.Item(n_Index).Value = Choose(n_Index + 1, s_Estado_Ina, s_Estado_Act, s_Estado_Blq)
  ...
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 1) = Choose(n_Index + 1, "verifica", "ordascen", "orddesce", "busqueda", "prelimin", "Imprimir")
    aElemento(n_Index, 2) = Choose(n_Index + 1, "Consulta de Cálculo " & s_TitleTable, "Ordenar Ascendente", "Ordenar Descendente", "Buscar " & s_TitleTable$, "Presentación Preliminar", "Imprimir")
  Next n_Index
  ...
  RecuperaRegistros "pdoano, sempvs ASC"
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fConsultaCalculo") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & o_PvsGratiConsul.Caption, vbExclamation
    Cancel = True
  End If
