...
Begin VB.Form fFormulaConcepto 
   ...
   End
End
Attribute VB_Name = "fFormulaConcepto"
...
Private Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT cxc.codcpc, cpc.descpc, cpr.secuencia, cxc.clasecpc, cxc.defaultcpc,"
  s_Sql = s_Sql & " cxc.impbolecpc, cxc.formulafun, cxc.imagenfun"
  s_Sql = s_Sql & " FROM plconceplanilla cxc, plconcepto cpc, plconceproceso cpr"
  s_Sql = s_Sql & " WHERE cxc.codcls='" & ps_ClsPlanilla & "'"
  s_Sql = s_Sql & " AND cxc.codcls=cpr.codcls"
  s_Sql = s_Sql & " AND cpc.codcpc=cpr.codcpc"
  s_Sql = s_Sql & " AND cpr.codproce='" & Trim(Right(cmbProcesos, 10)) & "'"
  s_Sql = s_Sql & " AND cpc.codcpc=cxc.codcpc"
  ...
  Select Case Index
   Case 0  ' Visualizar o analizar, eliminar registro
    If Not (dcaRegistro.Recordset.EOF Or dcaRegistro.Recordset.BOF) Then
      ...
      With fAbcFormulas
        If Not (dcaRegistro.Recordset.EOF Or dcaRegistro.Recordset.BOF) Then
          .gConcepto = dcaRegistro.Recordset!codcpc
          .gTipoConcepto = dcaRegistro.Recordset!clasecpc
          .Show
        End If
      End With
    End If
   ...
   Case 4, 5  ' Opciones de impresión
    ...
    If (dcaRegistro.Recordset.EOF Or dcaRegistro.Recordset.BOF) Then Beep: MsgBox "No Existen conceptos para Imprimir", vbExclamation: Exit Sub
    ...
    s_Sql = s_Sql & " AND cxp.codproce='" & Trim(Right(cmbProcesos, 10)) & "'"
...
Private Sub Form_Load()
  ...
  s_TitleTable = "Conceptos de Cálculo"
  
  ReDim aElemento(4, 10)
  For i = 0 To (UBound(aElemento, 1) - 1)
    aElemento(i, 0) = Choose(i + 1, "Sec", "Código", "Descripción", "Tipo")
    aElemento(i, 1) = Choose(i + 1, "secuencia", "codcpc", "descpc", "clasecpc")
    aElemento(i, 2) = Choose(i + 1, 380, 700, 3230.03, 350)
    aElemento(i, 3) = Choose(i + 1, vbCenter, vbLeftJustify, vbLeftJustify, vbCenter)
    aElemento(i, 4) = Choose(i + 1, "", "", "", "")
    aElemento(i, 5) = Choose(i + 1, False, False, False, False)
    aElemento(i, 6) = Choose(i + 1, True, True, True, True)
    aElemento(i, 7) = Choose(i + 1, "", "", "", "")
    aElemento(i, 8) = Choose(i + 1, dbgTop, dbgTop, dbgTop, dbgTop)
    aElemento(i, 9) = Choose(i + 1, 0, 0, 0, 0)
  Next i
  ...
  For i = 0 To 1
    ...
    tdbRegistro.Columns(3).ValueItems.Item(i).Value = Choose(i + 1, "C", "F")
    tdbRegistro.Columns(3).ValueItems.Item(i).DisplayValue = LoadPicture(gdl_Procedure.ps_PathImagen & Choose(i + 1, "constant", "funcion") & ".bmp")
  Next i
  ...
  For i = 0 To (UBound(aElemento, 1) - 1)
      aElemento(i, 1) = Choose(i + 1, "seleccio", "ordascen", "orddesce", "busqueda", "prelimin", "Imprimir")
      aElemento(i, 2) = Choose(i + 1, "Selecciona y Edita " & s_TitleTable, "Ordenar Ascendente", "Ordenar Descendente", "Buscar " & s_TitleTable$, "Presentación Preliminar", "Imprimir")
  Next i
  gdl_Procedure.ViewGrafics Me, cmdAction, aElemento
  ' Presenta Barra de Herramientas
  n_IndexTool = -1: panTool_Click 0
  
  ' Adiciono los tipos de calculo
  cmbProcesos.Clear
  cmbProcesos.Locked = False
  
  s_Sql = "SELECT codcls, codproce, desproce "
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcConcepPlanilla") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcConcepPlanilla.Caption, vbExclamation
    Cancel = True
  End If
