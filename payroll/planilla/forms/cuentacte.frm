...
Begin VB.Form fCuentaCorriente 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "cuentacte.frx":00D0
      End
   End
End
Attribute VB_Name = "fCuentaCorriente"
...
Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT CONCAT(numctacte, LPAD(CONVERT(numcuota, char(2)), 2, '0')) AS cPrimaryKey, tpoctacte, numctacte, "
  s_Sql = s_Sql & "numcuota , codcpc, codpdoprv, fectacte, indchecar, numchecar, codbco, codmon, "
  s_Sql = s_Sql & "(CASE codmon WHEN 'N' THEN cargo_mn ELSE cargo_me END) AS cargo, "
  s_Sql = s_Sql & "(CASE codmon WHEN 'N' THEN abono_mn ELSE abono_me END) AS abono, "
  s_Sql = s_Sql & "cargo_mn, abono_mn, cargo_me, abono_me, indgratifi, tpodscto, indprn, codpdocan, estadoctacte "
  s_Sql = s_Sql & "FROM plcuentacte "
  s_Sql = s_Sql & "WHERE codcls='" & ps_ClsPlanilla & "' "
  s_Sql = s_Sql & "AND codpsn='" & fSelPersonal.dcaRegistro.Recordset!codpsn & "' "
...
Private Sub Form_Load()
  ...
  Me.Height = 5610: Me.Width = 7515
  Me.Left = 800: Me.Top = 900
  ' Recupera parámetro
  gdl_Procedure.pl_RecordSelector = True
  
  ' Titulo del formulario y la Grilla
  s_TitleWindow = "Control de Cuenta Corriente"
  s_TitleTable = "Cuenta Corriente"
  
  ReDim aElemento(8, 10)
  For i = 0 To (UBound(aElemento, 1) - 1)
    aElemento(i, 0) = Choose(i + 1, "Número", "Cuota", "Periodo", "Mon", "Cargo", "Abono", "Fecha", "Ok")
    aElemento(i, 1) = Choose(i + 1, "numctacte", "numcuota", "codpdoprv", "codmon", "cargo", "abono", "fectacte", "estadoctacte")
    aElemento(i, 2) = Choose(i + 1, 900, 800, 800, 450, 900, 900, 900, 300)
    aElemento(i, 3) = Choose(i + 1, vbLeftJustify, vbLeftJustify, vbLeftJustify, vbCenter, vbRightJustify, vbRightJustify, vbLeftJustify, vbCenter)
    aElemento(i, 4) = Choose(i + 1, "", "", "", "", "standard", "standard", s_FormatoFecha, "")
    aElemento(i, 5) = Choose(i + 1, dbgMergeRestricted, False, False, False, False, False, False, False)
    aElemento(i, 6) = Choose(i + 1, True, True, True, True, True, True, True, True)
    aElemento(i, 7) = Choose(i + 1, "", "", "", "", "", "", "", "")
    aElemento(i, 8) = Choose(i + 1, dbgCenter, dbgTop, dbgTop, dbgTop, dbgTop, dbgTop, dbgTop, dbgTop)
    aElemento(i, 9) = Choose(i + 1, 0, 0, 0, 0, 0, 0, 0, 0)
  Next i
  ...
  tdbRegistro.Columns(7).ValueItems.Presentation = dbgNormal
  tdbRegistro.Columns(7).ValueItems.Translate = True
  For i = 0 To 1
    ' Moneda del registo
    tdbRegistro.Columns(3).ValueItems.Add Item
    tdbRegistro.Columns(3).ValueItems.Item(i).Value = Choose(i + 1, s_Codmon_mn, s_Codmon_me)
    tdbRegistro.Columns(3).ValueItems.Item(i).DisplayValue = Choose(i + 1, s_Codmon_mn_Txt, s_Codmon_me_Txt)
    ' Estado del registro
    tdbRegistro.Columns(7).ValueItems.Add Item
    tdbRegistro.Columns(7).ValueItems.Item(i).Value = Choose(i + 1, s_Estado_Act, s_Estado_Ina)
    tdbRegistro.Columns(7).ValueItems.Item(i).DisplayValue = LoadPicture(gdl_Procedure.ps_PathImagen & Choose(i + 1, "estadok", "estadnok") & ".bmp")
  Next i
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcCuentaCorriente") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcCuentaCorriente.Caption, vbExclamation
    Cancel = True
  End If
