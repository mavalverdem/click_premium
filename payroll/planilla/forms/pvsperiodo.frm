...
Begin VB.Form fPvsPeriodo 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "pvsperiodo.frx":00D0
      End
   End
End
Attribute VB_Name = "fPvsPeriodo"
...
Private Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  If s_OptRegistro = "pvsvacacio" Then
    s_Sql = "SELECT codpvs, descripvs, fechapvs, rembasbeneficio, estadopvs "
    s_Sql = s_Sql & "FROM plpvsperiodovac "
  Else
    s_Sql = "SELECT CONCAT(pdoano, '-', sempvs) AS cPrimaryKey, pdoano, sempvs, "
    s_Sql = s_Sql & "descripvs, mesini, mesfin, rembasbeneficio, estadopvs "
    s_Sql = s_Sql & "FROM plpvsperiodogra "
  End If
...
Private Sub dcaRegistro_MoveComplete(ByVal adReason As ADODB.EventReasonEnum, ByVal pError As ADODB.Error, adStatus As ADODB.EventStatusEnum, ByVal pRecordset As ADODB.Recordset)
  ...
  If s_OptRegistro = "pvsvacacio" Then
    If FormVisible("fAbcPvsPeriodo") Then
      If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
        fAbcPvsPeriodo.ShowScreen
      End If
    End If
  Else
    If FormVisible("fAbcPvsPeriodoGrati") Then
      If Not dcaRegistro.Recordset.EOF And Not dcaRegistro.Recordset.BOF Then
        fAbcPvsPeriodoGrati.ShowScreen
      End If
    End If
  End If
...
Private Sub Form_Load()
  ...
  Me.Left = 105: Me.Top = 600
  ' Recupera parámetro
  gdl_Procedure.pl_RecordSelector = True
  
  s_OptRegistro = fMenu.Tag
  ' Titulo del formulario y la Grilla
  s_TitleWindow = "Registro Periodo de Provisión de " & IIf(s_OptRegistro = "pvsvacacio", "Vacaciones", "Gratificaciones")
  s_TitleTable = "Periodo Provisión"
  
  ReDim aElemento(3, 10)
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 0) = Choose(n_Index + 1, "Código", "Descripción", "Ok")
    aElemento(n_Index, 1) = Choose(n_Index + 1, IIf(s_OptRegistro = "pvsvacacio", "codpvs", "cprimarykey"), "descripvs", "estadopvs")
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If s_OptRegistro = "pvsvacacio" Then
    If FormVisible("fAbcPvsPeriodo") Then
      Beep
      MsgBox "Primero debe cerrar la Pantalla de " & fAbcPvsPeriodo.Caption, vbExclamation
      Cancel = True
    End If
  Else
    If FormVisible("fAbcPvsPeriodoGrati") Then
      Beep
      MsgBox "Primero debe cerrar la Pantalla de " & fAbcPvsPeriodoGrati.Caption, vbExclamation
      Cancel = True
    End If
  End If
