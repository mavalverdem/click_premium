...
Begin VB.Form fAbcProcesoCalculo 
   BorderStyle     =   1  'Fixed Single
   ClientHeight    =   3405
   ClientLeft      =   2265
   ClientTop       =   375
   ClientWidth     =   7200
   Icon            =   "abcprocal.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin TabDlg.SSTab tabRegister 
      ...
      TabPicture(0)   =   "abcprocal.frx":000C
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "abcprocal.frx":0108
      End
   End
End
Attribute VB_Name = "fAbcProcesoCalculo"
...
Private s_ProcesosCalculo As String                     ' Codigo del registro
Private l_ProceBloqueo As Boolean                       ' Proceso bloqueado remuneración anterior
'[
...
If Me.Tag = s_MdoData_Ins Then
    gdl_Procedure.EditText "PK", txtCodigo, "", Me.Tag, False, fProcesoCalculo.dcaRegistro.Recordset!codproce.DefinedSize
    gdl_Procedure.EditText "AT", txtDescripcion, "", Me.Tag, False, fProcesoCalculo.dcaRegistro.Recordset!desproce.DefinedSize
    ...
    l_ProceBloqueo = False
Else
    gdl_Procedure.EditText "PK", txtCodigo, fProcesoCalculo.dcaRegistro.Recordset!codproce, Me.Tag, True, fProcesoCalculo.dcaRegistro.Recordset!codproce.DefinedSize
    gdl_Procedure.EditText "AT", txtDescripcion, gdl_Funcion.aTexto(fProcesoCalculo.dcaRegistro.Recordset!desproce), Me.Tag, False, fProcesoCalculo.dcaRegistro.Recordset!desproce.DefinedSize
    gdl_Procedure.EditOptionCheck "AT", optEstado(0), (fProcesoCalculo.dcaRegistro.Recordset!estadoproce = s_Estado_Act), Me.Tag, False
    gdl_Procedure.EditOptionCheck "AT", optEstado(1), (fProcesoCalculo.dcaRegistro.Recordset!estadoproce <> s_Estado_Act), Me.Tag, False
    l_ProceBloqueo = (fProcesoCalculo.dcaRegistro.Recordset!estadoproce = s_EstadoRemAper)
End If
...
Private Sub cmdAction_Click(Index As Integer)
  ...
  ' Valido que proceso no se encuentre bloqueado
  If l_ProceBloqueo And Index <> 0 Then Beep: MsgBox "Proceso No se puede Actualizar se encuentra Bloqueado", vbExclamation:  Me.Tag = s_MdoData_Vis: Exit Sub
  ...
  If MsgBox("¿ Estás Seguro de Eliminar el " & lblTitle & " '" & Trim$(txtDescripcion) & "' ?", vbCritical + vbYesNo + vbDefaultButton2) = vbYes Then
    ...
    s_ProcesosCalculo = Trim$(txtCodigo)
    
    '[ Inicio la conexión a la base de datos ]
    ps_StrgConnec = OpenConnection(ps_Servidor, ps_DataBase)
    ' Creo los arreglos de eliminacion
    a_Where = Array("codcls", "codproce")
    a_Valores = Array(ps_ClsPlanilla, s_ProcesosCalculo)
    a_Tipos = Array(TipoDato.Caracter, TipoDato.Caracter)
      
    gdl_Conexion.IniciaTransaccion    'Inicia transacción
    ' Elimino el registro
    If Not Records_Del("plproceso", a_Where, a_Valores, a_Tipos) Then GoTo Error
    gdl_Conexion.ConfirmaTransaccion  'Confirma transacción
    
    MsgBox "Se Elimino exitosamente " & lblTitle, vbInformation
    ' Refresco el Ado control y la grilla
    gdl_Procedure.RefreshAdoControl fProcesoCalculo.dcaRegistro, fProcesoCalculo.tdbRegistro, lblTitle
    ' Verifico si aun existen registros
    l_ExistRecord = ((fProcesoCalculo.dcaRegistro.Recordset.EOF And fProcesoCalculo.dcaRegistro.Recordset.BOF) Or fProcesoCalculo.dcaRegistro.Recordset.RecordCount = 0)
    If Not l_ExistRecord Then
      fProcesoCalculo.dcaRegistro.Recordset.Find ("codproce >= '" & s_ProcesosCalculo & "'")
      If fProcesoCalculo.dcaRegistro.Recordset.EOF Then fProcesoCalculo.dcaRegistro.Recordset.MoveLast
    Else
      Unload Me
    End If
  End If
...
Private Sub cmdMove_Click(Index As Integer)
  ...
  Select Case Index
   Case 0: fProcesoCalculo.dcaRegistro.Recordset.MoveFirst
   Case 1: If Not fProcesoCalculo.dcaRegistro.Recordset.BOF Then fProcesoCalculo.dcaRegistro.Recordset.MovePrevious
           If fProcesoCalculo.dcaRegistro.Recordset.BOF Then fProcesoCalculo.dcaRegistro.Recordset.MoveFirst
   Case 2: If Not fProcesoCalculo.dcaRegistro.Recordset.EOF Then fProcesoCalculo.dcaRegistro.Recordset.MoveNext
           If fProcesoCalculo.dcaRegistro.Recordset.EOF Then fProcesoCalculo.dcaRegistro.Recordset.MoveLast
   Case 3: fProcesoCalculo.dcaRegistro.Recordset.MoveLast
  End Select
...
Private Sub cmdUpdate_Click()
  ...
  s_ProcesosCalculo = txtCodigo
    
  ' Creo los arreglos para la actualización
  a_Campos = Array("codcls", "codproce", "desproce", "estadoproce", IIf(Me.Tag = s_MdoData_Ins, "usrcre", "usrmdf"), IIf(Me.Tag = s_MdoData_Ins, "fyhcre", "fyhmdf"))
  ...
  If Me.Tag = s_MdoData_Ins Then
    If Not Records_Ins("plproceso", a_Campos, a_Valores, a_Tipos) Then GoTo Error
  Else
    If Not Records_Upd("plproceso", a_Campos, a_Valores, a_Tipos, a_Where) Then GoTo Error
  End If
  gdl_Conexion.ConfirmaTransaccion ' Confirma transacción
...
Private Sub Form_Load()
  ...
  s_TitleWindow = "Actualización Procesos de Cálculo"
  lblTitle = "Proceso de Cálculo"
  ' Coloco el puntero en espera
  gdl_Procedure.PunteroEnEspera
