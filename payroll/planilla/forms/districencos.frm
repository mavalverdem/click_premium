...
Begin VB.Form fDistribucionCenCosto 
   ...
   Begin Threed.SSPanel panToolBar 
      ...
      Begin Threed.SSCommand cmdAction 
         ...
         Outline         =   0   'False
         AutoSize        =   2
         Picture         =   "districencos.frx":00D0
      End
   End
End
Attribute VB_Name = "fDistribucionCenCosto"
...
Sub RecuperaRegistros(ByVal s_Orden As String)
  ...
  s_Sql = "SELECT dxc.codcco, cco.detcco, dxc.porcentaje "
  s_Sql = s_Sql & "FROM plcencospro dxc, " & ps_DaBasCon & ".cocco cco "
  s_Sql = s_Sql & "WHERE dxc.codcls='" & ps_ClsPlanilla & "' "
  s_Sql = s_Sql & "AND dxc.codpdo='" & Trim(o_SelDisCenCosto.txtPeriodo.Text) & "' "
  s_Sql = s_Sql & "AND dxc.codpsn='" & o_SelDisCenCosto.dcaRegistro.Recordset!codpsn & "' "
  s_Sql = s_Sql & "AND cco.codcco=dxc.codcco "
  ...
  Select Case Index
   ...
   Case 5 ' Busqueda de registro
    If Not (dcaRegistro.Recordset.EOF Or dcaRegistro.Recordset.BOF) Then
      ...
      gn_ColBusqueda = (tdbRegistro.Columns.Count - 2)
      fBusqueda.Show vbModal
    End If
   Case 6, 7  ' Opciones de impresión
    ' Parametros de Impresión
    gdl_Procedure.ps_ReportTitle = "DISTRIBUCIÓN POR CENTRO DE COSTO"
    gdl_Procedure.ps_ReportName = "rptremexepci"
    ReDim aElemento(3, 3): ReDim aElementos(2)
    ' Parametros del Reporte
    aElemento(0, 0) = ps_CodEmpresa
    aElemento(0, 1) = o_SelDisCenCosto.tdbRegistro.Columns(0).DataField & " ASC"
    aElemento(0, 2) = ""
    ...
    aElemento(2, 2) = "Periodo;" & Trim(o_SelDisCenCosto.txtPeriodo.Text) & " - " & Trim(o_SelDisCenCosto.lblHelp(0).Caption) & ";true"
    ...
    s_Sql = s_Sql & " WHERE rex.codpdo='" & Trim(o_SelDisCenCosto.txtPeriodo.Text) & "'"
    s_Sql = s_Sql & " AND psn.codcls='" & ps_ClsPlanilla & "'"
    s_Sql = s_Sql & " AND psn.codpsn='" & o_SelDisCenCosto.tdbRegistro.Columns(0).Text & "'"
...
Private Sub Form_Load()
  ...
  Me.Left = 2690: Me.Top = 770
  
  ' Bloqueo los valores del periodo seleccionado
  o_SelDisCenCosto.txtPeriodo.Locked = True
  o_SelDisCenCosto.txtPeriodo.BackColor = &HC7D8E0
  o_SelDisCenCosto.txtPeriodo.ForeColor = &HC00000
  o_SelDisCenCosto.cmdHelp(0).Enabled = False
  ' Recupera parámetro
  gdl_Procedure.pl_RecordSelector = True
  
  ' Titulo del formulario y la Grilla
  s_TitleWindow = "Distribución de Centro de Costo"
  s_TitleTable = "Centro de Costo"
  
  ReDim aElemento(3, 10)
  For n_Index = 0 To (UBound(aElemento, 1) - 1)
    aElemento(n_Index, 0) = Choose(n_Index + 1, "Código", "Descripción", "Porcentaje")
    aElemento(n_Index, 1) = Choose(n_Index + 1, "codcco", "detcco", "porcentaje")
    aElemento(n_Index, 2) = Choose(n_Index + 1, 700, 2916.03, 1050)
    aElemento(n_Index, 3) = Choose(n_Index + 1, vbLeftJustify, vbLeftJustify, vbRightJustify)
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  If FormVisible("fAbcDistribuCenCos") Then
    Beep
    MsgBox "Primero debe cerrar la Pantalla de " & fAbcDistribuCenCos.Caption, vbExclamation
    Cancel = True
    Exit Sub
  End If
  ' Restauro los valores del periodo seleccionado
  o_SelDisCenCosto.txtPeriodo.Locked = False
  o_SelDisCenCosto.txtPeriodo.BackColor = &HFFFFFF
  o_SelDisCenCosto.txtPeriodo.ForeColor = &HC00000
  o_SelDisCenCosto.cmdHelp(0).Enabled = True
